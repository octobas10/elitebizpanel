...
class IndexProcessController extends Controller{
	public function actionIndex(){
		...
		$_REQUEST['address'] = urldecode($_REQUEST['address']);
		$_REQUEST['dob'] = urldecode($_REQUEST['dob']);
		$_POST = $_REQUEST;
		$process = 'organic';
		...
		if($affiliate_status!='TestMode'){
			if($affiliate_cap){
				...
				foreach($lenderlist as $lender){
					...
					$check_submission_accept_cap = $lender_details_model->check_acceptCap($lender_id);
				...
				foreach($lenderlist as $lender){
					...
					if($lender->ping_url_test!='' || $lender->ping_url_live !='' ){
						...
						$ping_id = $cm->setLenderTransactionLog($lendername,$lenders_response[$lender_id]['ping_price'],$post_request=null,$post_status=null,$post_response=null,$lenders_response[$lender_id]['ping_time'],$lenders_response[$lender_id]['ping_request'],$lenders_response[$lender_id]['ping_status'],$lenders_response[$lender_id]['ping_response'],$redirect_url=null,$ping_id=null,$lender_id);
				...
				//echo '<pre>';print_r($Max_Price_Ping_Accepted_Lender);exit;
				/** =================END====================  */
				...
				if(!empty($static_price_lenderlist)){
					...
					foreach($static_price_lenderlist as $static_price_lender){
						...
						$confirmation_id = '';
						$ping_response = $static_price_lender->ping_response;
						$post_url = $static_price_lender->post_url_live;
						$lender_status = $lender->status;
						/** Send Post Data to Static Price Lender */
						$post_responses=call_user_func_array(array($LenderClassName,$methodName),array($arg1,$arg2,$arg3,$ping_response,$post_url,$lender_status));
						...
						$cm->setLenderTransactionLog($lendername,$post_responses['post_price'],$post_responses['post_request'],$post_responses['post_status'],$post_responses['post_response'],$post_responses['post_time'],null,null,null,$post_responses['redirect_url'],$ping_id = null,$lender_id);
				...
				if($Max_Price_Ping_Accepted_Lender['ping_price']!='' && $static_lender_accept_status==false){
					...
					$ping_response = $Max_Price_Ping_Accepted_Lender['ping_response'];
					...
					$lender_aff_trans_model->lender_post_status_update($lender_id,$promo_code,$post_responses['post_status'],$post_price);
					/** Update Post Log in LenderTransaction Table*/
					$cm->setLenderTransactionLog($lendername,$post_price,$post_responses['post_request'],$post_responses['post_status'],$post_responses['post_response'],$post_responses['post_time'],$ping_request=null,$ping_status=null,$ping_response=null,$post_responses['redirect_url'],$lenders_response[$max_price_lender_id]['ping_id'],$lender_id);
				}
				/** ===============END=================*/
