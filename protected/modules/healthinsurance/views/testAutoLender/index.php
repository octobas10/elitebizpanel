					...
					<div class="widget-body">
						<div class="widget-main">
							<div class="">
								...
								<?php echo $form->dropDownList($AffiliateUser_model,'id',CHtml::listData(AffiliateUser::model()->findAll(),'id','id'),array('--Select-')); ?>
							</div>
							<div class="">
								<label for="form-field-8">Lender <span class="testcash_lender_label">(Only Test Mode Status Lenders)</span></label>
								<?php echo $form->dropDownList($LenderDetails_model, 'name', CHtml::listData(LenderDetails::model()->findAll(array("condition"=>"status = 0")), 'id', 'name')); ?>
							</div>
							<div class="">
					...
					<div class="widget-body">
						<div class="widget-main">
							...
							<div class="">
								...
								<input type="text" value="<?php echo trim($ssn);?>" class="t3InputText required" name="ssn" id="ssn">
							</div>
							<div class="">
							...
							<div class="">
								...
								<input type="text" class="t3InputText" value="<?php echo trim($mobile);?>" name="mobile" id="mobile">
							</div>
							<div class="">
					...
					<div class="widget-body">
						<div class="widget-main">
							...
							<!--<div class="">
                                ...
                                <input type="radio" name="pingpost" value="ping" id="ping">Only Ping
                                <input type="radio" name="pingpost" value="post" id="post">Only Post
                                <input type="radio" name="pingpost" value="both" id="both" checked="checked">Both
                            </div>-->
						</div>
					</div>
				</div>
...
<thead>
	<tr><th><?php echo 'count:'.count($posts); ?></th></tr>
    <tr>
    <!-- <th>Sr#</th> -->
    <th>Date</th>
...
<?php $i=1; foreach ($posts as $aff_trans) {?>
	...
	<td>
		...
		<p><b>Post Response</b></p>
		<p style="word-wrap:break-word;width:400px"><?php echo htmlentities($len_trans['post_response']); ?>
		</p>
		<?php }?>
...
<div class="row-fluid">
	<div class="span12">
		<table id="posts" class="table table-striped table-hover table-bordered table-condensed">
			...
			<tbody>
				<?php $connection=Yii::app()->dbHealthinsurance; foreach ($posts as $row) { ?>
				<tr class="post">
					<td>
						<?php echo $row['date']; ?>
					</td>
					...
					<td>
						<?php $command=$connection->createCommand('SELECT * FROM healthinsurance_lender_transactions WHERE affiliate_transactions_id ='.$row['id']); $res = $command->queryAll(); foreach ($res as $row) { ?>
						<table class="table table-striped table-hover table-bordered table-condensed">
							<tr>
								<td>
									<?php echo $row[ 'name']; ?>
