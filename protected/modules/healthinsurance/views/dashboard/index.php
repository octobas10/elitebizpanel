...
<div class="row-fluid">
	<div class="span9">
		<?php
			$this->beginWidget('zii.widgets.CPortlet',array('title' => '<span class="icon-picture"></span>Campaign Performance - Auto Finance'));
			if(is_array($leads)){?>
					...
					<?php
						foreach($lenders as $lender_id){
							$total_pings=array_sum(array_column($lenders_statistics[$lender_id],'ping_sent'));
						  	$total_accepted_pings = array_sum(array_column($lenders_statistics[$lender_id],'ping_accepted'));
							echo '<th class="header" align=right>'.$total_pings.'</th>'.'<th class="header" align=right>'.$total_accepted_pings.'</th>';
						}
						?>
				</tr>
