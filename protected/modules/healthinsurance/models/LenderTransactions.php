<?php
/**
 * This is the model class for table "healthinsurance_lender_transactions".
 *
 * The followings are the available columns in table 'healthinsurance_lender_transactions':
 * @property integer $id
 ...
 */
class LenderTransactions extends HealthinsuranceActive{
	/**
	...
	public function tableName(){
		return 'healthinsurance_lender_transactions';
	}
	/**
	 *
	...
	public function browselendertransction(){
		...
		->select('id,affiliate_transactions_id,lender_name,date,ping_request,ping_response,ping_status,ping_price,post_request,post_response,post_status,post_price')
	...
	public function exit_url($affiliate_trans_id){
		...
		$exiturl = Yii::app()->dbHealthinsurance->createCommand()
		->select('exit_url')
		->from('healthinsurance_lender_transactions')
		->where($where)
		->queryScalar();
	...
	public function get_Lender_Pigned_With_This_Ping_Id($affiliate_trans_id){
		...
		$dbCommand = Yii::app()->dbHealthinsurance->createCommand()->setFetchMode(PDO::FETCH_OBJ)
		...
		->from('healthinsurance_lender_transactions a')
		...
		//mail('octobas@gmail.com','Healthinsurance : multiple ping accepted lenders',$qry);
		return $dataReader;
	}
	/**
	...
	public function lender_reports(){
		...
		$rawData1 = Yii::app()->dbHealthinsurance->createCommand()
		...
		->from('healthinsurance_lender_transactions as a')
		->where($where)
		->group($groupby)
		->order($order)
		->queryAll();
		//echo $rawData1->getText();exit;
		$lender_array = [];
