<?php
class FeedLenderTransactions extends HealthinsuranceActive {
    /**
    ...
    public function tableName() {
        return 'healthinsurance_feed_lender_transactions';
    }
    /**
...
//  		echo '<pre>'.$where.'</pre>';exit;
		...
		return $rawData = Yii::app()->dbHealthinsurance->createCommand()
	    ->select('id,feed_lender_name,date,request,full_response,response,')
	    ->from('healthinsurance_feed_lender_transactions')
		->where($where)
		->order($order)
	    ->queryAll();
	}
