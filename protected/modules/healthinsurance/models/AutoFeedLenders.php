...
/**
 * This is the model class for table "healthinsurance_feed_lenders".
 *
 * The followings are the available columns in table 'healthinsurance_feed_lenders':
 * @property integer $id
 ...
 */
class AutoFeedLenders extends HealthinsuranceActive
{
	/**
	...
	{
		return 'healthinsurance_feed_lender';
	}
	...
	public function getvendorslist(){
		extract($_REQUEST);
		$update = Yii::app()->dbHealthinsurance->createCommand()->update('healthinsurance_feed_lenders', array('paused_vendor'=>$val),'id=:id',array(':id'=>$id));
		if($val!=""){
			return $val;
		}
	...
	public function check_caps($feed_lender_name,$promo_code = false){
		...
		$feedlenderlist = Yii::app()->dbHealthinsurance->createCommand()
		...
		->from('healthinsurance_feed_lenders')
		...
		if($capdate != @date('Y-m-d')){
			Yii::app()->dbHealthinsurance->createCommand()->update('healthinsurance_feed_lenders', array(
				'capdate'=>@date('Y-m-d'),
		...
		$intervalcount = Yii::app()->dbHealthinsurance->createCommand()
