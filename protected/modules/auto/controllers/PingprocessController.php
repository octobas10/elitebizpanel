...
class PingprocessController extends Controller
{
	public function actionIndex()
	{
		...
		if ($reqCount < Yii::app()->params['auto_ping_cap']) {
			$_POST = $_REQUEST;
			$process = 'inorganic';
			...
			if ($lead_mode == 0) {
				$cm->setAffiliatePingResponse(1, '1.50', '0.00', '00');
			}
				...
				if ($affiliate_cap) {
					...
					foreach ($lenderlist as $lender) {
						...
						if ($lender->ping_url_test != '' || $lender->ping_url_live != '') {
							...
							//mail('tony.elitecashwire@gmail.com','from pingprocesscontroller 143',"==>".$lender_id."<==");
