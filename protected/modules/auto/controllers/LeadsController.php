...
class LeadsController extends Controller{
	...
	public static function actionAffiliateTransaction($data,$process='organic'){
		...
		if ($ping_id !='' && Yii::app()->session['ping_type'] == 'post') {
			...
			$model_aff_trnsaction->ping_request  = '';
			$model_aff_trnsaction->ping_response  = '';
			$model_aff_trnsaction->ping_status  = 0;
			$model_aff_trnsaction->ping_price  = 0.00;
			$model_aff_trnsaction->vendor_ping_price   = 0.00;
			$model_aff_trnsaction->ping_time   = 0;
			...
			$model_aff_trnsaction->post_response = '';
			$model_aff_trnsaction->post_status = 0;
			$model_aff_trnsaction->post_price = 0.00;
			$model_aff_trnsaction->vendor_post_price  = 0.00;
			$model_aff_trnsaction->post_time   = 0;
			$model_aff_trnsaction->redirect   = '';
			$model_aff_trnsaction->is_returned   = 0;
			...
			$model_aff_trnsaction->ping_time   = 0;
			//post
			$model_aff_trnsaction->post_request = '';
			$model_aff_trnsaction->post_response = '';
			$model_aff_trnsaction->post_status = 0;
			$model_aff_trnsaction->post_price = 0.00;
			$model_aff_trnsaction->vendor_post_price  = 0.00;
	...
	*/
	public static function actionCheckDuplicate($data,$process = 'organic'){
		...
		if(!empty($count) && $count > 0){
			...
			if(Yii::app()->request->getParam('ping_id')){
				...
				$model->url = 'http://eliteautocash.com';
				$model->user_agent ='';
				$model->redirect ='0';
				$model->redirect_ip ='';
				$model->redirect_url ='';
	...
	public static function actionCheckPingDuplicate($data,$process='organic'){
		...
		if(!empty($count) && $count > 0){
			...
			$model->is_organic = ($process == 'organic') ? 1 : 0;
			$model->sub_id = isset($data['sub_id']) ? $data['sub_id'] : '';
			...
			$model->state = isset($data['state']) ? $data['state'] : '';
			$model->zip = isset($data['zip']) ? $data['zip'] : '';
			$model->gender = isset($data['gender']) ? $data['gender'] : '';
			$model->ssn = isset($data['ssn']) ? $data['ssn'] : '';
			$model->is_rented = isset($data['is_rented']) ? $data['is_rented'] : '';
			$model->employer = isset($data['employer']) ? $data['employer'] : '';
			$model->job_title = isset($data['job_title']) ? $data['job_title'] : '';
			$model->car_year = isset($data['car_year']) ? $data['car_year'] : '';
			$model->car_make = isset($data['car_make']) ? $data['car_make'] : '';
			$model->car_model = isset($data['car_model']) ? $data['car_model'] : '';
			$model->car_trim = isset($data['car_trim']) ? $data['car_trim'] : '';
			$model->ipaddress = isset($data['ipaddress']) ? $data['ipaddress'] : '';
			$model->url = isset($data['url']) ? $data['url'] : '';
			$model->user_agent = isset($data['user_agent']) ? $data['user_agent'] : '';
			$model->redirect = isset($data['redirect']) ? $data['redirect'] : '';
			$model->redirect_ip = isset($data['redirect_ip']) ? $data['redirect_ip'] : '';
			$model->lead_status = 0;
			$model->is_returned = 0;
			//$model->sub_date = 0;
			$model->save();
