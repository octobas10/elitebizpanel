...
class EtnamericaController extends Controller {
	...
	public static function returnPingData($p1 = false, $p2 = false, $p3 = false, $status = 0){
		...
		$fields = array (
			'Key' => $p1 ? $p1 : 'EF8m9Cyu8PyHqjyKECqG8C_I8grLEFYG8geKECvcScYu8cRL9OSLlWr64Yff',
			'API_Action' => 'pingPostLead',
			'Mode' => 'ping',
			'Return_Best_Price' => '1',
			'TYPE' => $p2 ? $p2 : '20',
			'SRC' => $p3 ? $p3 : 'EliteMate-Auto-Finance',
			'Zip' => $ZIP_POSTAL_CODE,
			'SSN' => $SSN,
	...
	public static function returnPingResponse($ping_response){
		...
		if (trim ($result[1]) == 'Matched' || trim(strtolower($result[1])) == 'matched'){
	...
	public static function sendPostData($parameter1,$parameter2,$parameter3,$ping_response,$post_url,$status){
		...
		$fields = array (
			'Key' => $parameter1 ? $parameter1 : 'EF8m9Cyu8PyHqjyKECqG8C_I8grLEFYG8geKECvcScYu8cRL9OSLlWr64Yff',
			'API_Action' => 'pingPostLead',
			'Mode' => 'post',
			'Lead_ID' => trim($PingId),
			'TYPE' => $parameter2 ? $parameter2 : '20',
			'IP_Address' => $LEAD_SOURCE_IP,
			'SRC' => $parameter3 ? $parameter3 : 'EliteMate-Auto-Finance',
		...
		if(trim($success[1]) == 'Matched' || trim(strtolower($success[1])) == 'matched'){
			$post_status = '1';
