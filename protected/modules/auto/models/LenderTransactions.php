<?php
/**
 * This is the model class for table "auto_lender_transactions".
 *
 * The followings are the available columns in table 'auto_lender_transactions':
 * @property integer $id
 ...
 */
class LenderTransactions extends CActiveRecord{
	/**
	...
	public function tableName(){
		return 'auto_lender_transactions';
	}
	/**
	 *
	...
	public function browselendertransction(){
		...
		return $rawData = Yii::app()->db->createCommand()
	...
	public function exit_url($affiliate_trans_id){
		...
		$exiturl = Yii::app()->db->createCommand()
		->select('exit_url')
		->from('auto_lender_transactions')
		->where($where)
		->queryScalar();
	...
	public function get_Lender_Pigned_With_This_Ping_Id($affiliate_trans_id){
		...
		$dbCommand = Yii::app()->db->createCommand()->setFetchMode(PDO::FETCH_OBJ)
		...
		->from('auto_lender_transactions a')
		...
		//$qry = $dbCommand->getText();
		//mail('octobas@gmail.com','Auto : multiple ping accepted lenders',$qry);
		return $dataReader;
	}
	/**
	...
	public function lender_reports(){
		...
		->from('auto_lender_transactions as a')
		->where($where)
		->group($groupby)
		->order($order)
		->queryAll();
