<?php
class LenderAffiliateTransaction extends CActiveRecord{
	public $lender,$ping_sent,$post_sent,$ping_accepted, $post_accepted, $lead_price, $total, $last15dayes_accepted, $last15dayes_submission, $average_ping_price, $revenue =  0;
	...
	public function tableName(){
		return 'auto_lenders_affiliates_transactions';
	}
	/**
	...
	public function check_lender_affiliate_cap($promo_code,$lender_id){
		...
		$vendor_lender_cap = Yii::app()->db->createCommand()
		...
		//$lender_data = LenderDetailids::model()->findByPK($lender_id);
		$where1 = "lender_id = $lender_id AND promo_code= '$promo_code' AND DATE(date) ='".date('Y-m-d')."'";
		$submission_of_lender = Yii::app()->db->createCommand()
		...
		$time_at_last_lead_sent = Yii::app()->db->createCommand()
	...
	public function lender_ping_status_update($lender_id,$promo_code,$ping_status,$price){
		...
		//mail('tony.elitecashwire@gmail.com','from pingprocesscontroller 87777',"==>".$lender_id."<==");
		$sql1="INSERT INTO auto_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),1,$ping_status,0,0,now()) on duplicate key update ping_sent=ping_sent+1 $accept_query, last_inserted_datetime=NOW()";
	...
	public function lender_post_status_update($lender_id,$promo_code,$post_status,$price,$static_price_direct_post=false){
		...
		if($static_price_direct_post==false){
			$sql1="INSERT INTO auto_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),1,1,1,$post_status,now()) on duplicate key update post_sent=post_sent+1 $accept_query, last_inserted_datetime=NOW()";
		}elseif($static_price_direct_post==true){
			$sql1="INSERT INTO auto_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),0,0,1,$post_status,now()) on duplicate key update post_sent=post_sent+1 $accept_query, last_inserted_datetime=NOW()";
		}
		$command=$connection->createCommand($sql1);
		$dataReader=$command->query();
	}
