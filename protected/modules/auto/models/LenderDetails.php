<?php
/**
 * This is the model class for table "auto_lender_details".
 *
 * The followings are the available columns in table 'auto_lender_details':
 * @property integer $id
 ...
 */
class LenderDetails extends CActiveRecord{
	/**   
	...
	public function rules(){
		...
		return array(
		   array('user_name,name','required'),
			...
			array('email,status,first_name,last_name,company_name,phone,submission_cap,hourly_submission_cap,accepted_cap,note','length'),
		   ...
		   array('name','unique', 'message'=>'{attribute}:{value} already exists!'),
			...
			array('id,name,static_lead_price,parameter1,parameter2,parameter3,ping_url_test,ping_url_live, posting_timelimit,active_mode,status','safe', 'on'=>'search'),
		);
	}
	/**
	...
	public function actionPausevendorAjax(){
		...
		$sql = "UPDATE auto_lender_details SET paused_vendor  = '$val' WHERE id=$id";
	...
	public function check_acceptCap($lender_id) {
		...
		$cmd = Yii::app()->db->createCommand()
			...
			->from('auto_lenders_affiliates_transactions')
			->where($where)
			->limit('1');
		$row = $cmd->queryRow();
	...
	/*public function check_acceptCap($lender_name) {
		...
		$cmd = Yii::app()->db->createCommand()
			...
			->limit('1');
		$sub_accept_cap = $cmd->queryRow();	
		$len_submission_cap = $sub_accept_cap['submission_cap'];
		$len_hourly_submission_cap = $sub_accept_cap['hourly_submission_cap'];
		$len_accepted_cap = $sub_accept_cap['accepted_cap'];
		
		// TOTAL SUBMISSION AND TOTAL ACCEPTED FOR LENDER TODAY 
		$where = "lender= '$lender_name' AND DATE(date) = DATE(CURTIME())";
		$cmd = Yii::app()->db->createCommand()
			->select('SUM(`ping_sent`) as total_sub,SUM(`post_accepted`) as total_accept')
			->from('auto_lenders_affiliates_transactions')
			->where($where)
			->limit('1');
		$total_sub_accept_cap = $cmd->queryRow();	
		$sub_cap = $total_sub_accept_cap['total_sub'];
		$accep_cap = $total_sub_accept_cap['total_accept'];
		// TOTAL HOURLY SUBMISSION CAP
		$where = "lender = '$lender_name' AND HOUR(`last_inserted_datetime`) = HOUR(CURTIME()) AND DATE(date) = DATE(CURTIME())";
		$cmd = Yii::app()->db->createCommand()
			->select('SUM(`ping_sent`) as total_hourly_sub')
			->from('auto_lenders_affiliates_transactions')
			->where($where)
			->limit('1');
		$totalsub_hourly_cap = $cmd->queryRow();	
		$hourly_sub_cap = $totalsub_hourly_cap['total_hourly_sub'];
		
		// CHECK TRAFFIC IN AND OUT TIME FOR THE LENDER, IF RECORD SET THEN SEND TRUE
		$where = "user_name= '$lender_name' AND (NOT FIND_IN_SET(DAYOFWEEK(DATE(CURTIME())),traffic_out_days) OR traffic_out_days='' OR traffic_out_days IS NULL) AND (HOUR(CURTIME()) BETWEEN traffic_in_start_time AND traffic_in_end_time OR (traffic_in_start_time='' AND traffic_in_end_time='') OR (traffic_in_start_time IS NULL AND traffic_in_end_time IS NULL))";
		$cmd = Yii::app()->db->createCommand()
			->select('id')
			->from('auto_lender_details')
			->where($where)
		...
		if(empty($InoutDays['id'])){
			...
			return true;
		}else if($len_hourly_submission_cap != '-1' && $len_hourly_submission_cap <= $hourly_sub_cap){
			return false;
		}else if($len_submission_cap != '-1' && $len_submission_cap <= $sub_cap){
			return false;
		}else if($len_accepted_cap != '-1' && $len_accepted_cap <= $accep_cap){
			return false;
		}else{
			return true;
		}
	...
	public function GetAllLenders(){
		$lender_details = Yii::app()->db->createCommand()
		->select('id,name')
		->from('auto_lender_details')
		->queryAll();
		$lenders = [];
