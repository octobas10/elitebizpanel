		...
		<table id="posts" class="table table-striped table-hover table-bordered table-condensed">
			...
			<tbody>
				...
				<tr class="post">
					...
					<td>
						<?php $command=$connection->createCommand('SELECT * FROM auto_lender_transactions WHERE affiliate_transactions_id ='.$row['id']); $res = $command->queryAll(); foreach ($res as $row) { ?>
						<table class="table table-striped table-hover table-bordered table-condensed">
							<tr>
								<td>
									<?php echo $row[ 'name']; ?>
