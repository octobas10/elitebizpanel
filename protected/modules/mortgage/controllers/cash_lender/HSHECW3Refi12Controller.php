<?php
class HSHECW3Refi12Controller extends Controller {
    ...
    public static function returnPingData($p1 = false, $p2 = false, $p3 = false, $status = 0) {
        ...
        if($mort_type == 'REFI' ){
            ...
            $accapted_promocodes = [14];
            if(!in_array($promo_code,$accapted_promocodes)){
                $ping_response = $promo_code .' Not Allowed';
                $purchase = false;
            }else if(!in_array($state,$state_included)){
                $ping_response = $state .' Not Allowed';
                $purchase = false;
            }else if($accepted_cap_count > 150){
                $ping_response = 'Daily Cap 150 Met:REFI';
                $purchase = false;
            }else if($loan_amount < '150000'){
                $ping_response = 'loan amount should be less than $150,000';
                $purchase = false;
            }else if($credit_rat > '1'){
                $ping_response = 'Credit Rating only Good/Excellent Allowed';
                $purchase = false;
            }else if($ltv_percentage >= '90'){
                $ping_response = 'Ltv should not be more than 90%';
                $purchase = false;
            }else if($prop_desc > '1'){
