...
class HSHController extends Controller {
    ...
    public static function returnPingData($p1 = false, $p2 = false, $p3 = false, $status = 0) {
        ...
        if($mort_type == 'NEWHOME'){
            ...
            $Username = '';$purchase = true;
            if($credit_rating >= '3'){ // BAD CREDIT
                $purchase = true;
                ...
                if($tier7_cap_count > 1){
                    $ping_response = 'Daily Cap 1 Met for com_LeadAPI_Fair :'.$mort_type;
                    $purchase = false;
                }else if($additional_cash < '10000'){
                    $ping_response = 'Cash Out should not be less than $10,000 You sent: '.$additional_cash;
                    $purchase = false;
                }else if($loan_amount < '350000'){
                    $ping_response = 'loan amount should be more than $350,000 You sent: '.$loan_amount;
                    $purchase = false;
