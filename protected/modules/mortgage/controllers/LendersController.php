...
class LendersController extends Controller{
	...
	public function actionPausevendorAjax(){
		...
		$sql2 = "UPDATE mortgage_lender_details SET paused_vendor  = '$val' WHERE id=$id";
	...
	public function actionLenderreport(){
		...
		foreach ($lender_price_leads as $lender_name => $transactions){
			foreach ($transactions as $key => $trans){
				foreach ($trans as $key => $trans){
					$mortgage_lead_type = $trans['mortgage_lead_type'];
			...
			$lender_price_leads[$lender_name]['mortgage_lead_type'] =  $mortgage_lead_type;
		...
		//echo '<pre>';print_r($lender_price_leads);exit;
		/*** SEARCHED DATA ARRAY **/
		$searched_data = array();
