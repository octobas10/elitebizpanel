...
class PingprocessController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($reqCount < Yii::app()->params['mortgage_ping_cap']) {
            $_POST = $_REQUEST;
            $process = 'inorganic';
            ...
            if ($affiliate_status != 'TestMode') {
                if ($affiliate_cap) {
                    ...
                    if ($list) {
                        if ($promo_code == '1') {
                            $lenderlist = $lender_details_model->findAll(array("condition" => " id IN (" . $list . ") AND lender_pingpost_type=0"));
                        } else {
