...
class AffiliateTransactions extends MortgageActive {
	/**
	...
	public function tableName() {
		return 'mortgage_affiliate_transactions';
	}
	/**
    ...
    */
	...
	public function browse_aff_transction() {
		...
		$aff_tran_obj=Yii::app()->dbMortgage->createCommand()
			->select('sum(ping_sent) as submissions,sum(post_accepted) as accepted')
			->from('mortgage_lenders_affiliates_transactions t')
			->where($where)
			->queryAll();
		return $aff_tran_obj;
	}
	...
	public function acceptLaststate15days(){
		...
		$accept15days = Yii::app()->dbMortgage->createCommand()
	    	->select(array('count(au.id) as cnt1', 'au.state as state'))
		    ->from('mortgage_affiliate_transactions af')
		    ->join('mortgage_submissions au', 'af.customer_id=au.id')
		    ->where($where)
			->group('state')
			->order('cnt1 DESC')
			->limit(15)
	...
	public function leads_submitted_per_affiliate_last15days(){
		...
		$leads15days = Yii::app()->dbMortgage->createCommand()
	...
	public function log_redirect($affiliate_trans_id,$process) {
    	...
    	/** ---------------------- Invalid Affiliate Transaction Id ---------------------- */
	 	$invalid_tras_id=Yii::app()->dbMortgage->createCommand()
    		->select('id')
    		->from('mortgage_affiliate_transactions')
		...
		/** ------------------------- Unaccepted Lead -------------------------------------- */
		$unaccept_tras_id=Yii::app()->dbMortgage->createCommand()
			->select('id')
			->from('mortgage_affiliate_transactions')
		...
		/** -------------------------- Duplicate Redirection ------------------------------ */
	 	$dup_redirect_tras_id=Yii::app()->dbMortgage->createCommand()
    		->select('id')
    		->from('mortgage_affiliate_transactions')
		...
		$customer_id = $AffiliateTransactions->customer_id;
	 	...
	 	$valid_redirect_tras_id=Yii::app()->dbMortgage->createCommand()->update('mortgage_affiliate_transactions', array(
			'redirect'=>'yes',
		), 'id=:id', array(':id'=>$affiliate_trans_id));

	 	/** -------------------- Update submissions table "redirect=yes" ----------------------------------- */
		$submission_redirect=Yii::app()->dbMortgage->createCommand()->update('mortgage_submissions', array(
			'redirect'=>'yes',
	...
	public function search_posted_leads(){
		...
		->from('mortgage_affiliate_transactions as a')
		->join('mortgage_affiliate_user as b','a.promo_code = b.id')
		->where($where)
		->group($groupby)
		->order($order);
