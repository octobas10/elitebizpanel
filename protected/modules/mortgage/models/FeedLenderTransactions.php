<?php
class FeedLenderTransactions extends MortgageActive {
    /**
    ...
    public function tableName() {
        return 'mortgage_feed_lender_transactions';
    }
    /**
...
//  		echo '<pre>'.$where.'</pre>';exit;
		...
		return $rawData = Yii::app()->dbMortgage->createCommand()
	    ->select('id,feed_lender_name,date,request,full_response,response,')
	    ->from('mortgage_feed_lender_transactions')
		->where($where)
		->order($order)
	    ->queryAll();
	}
