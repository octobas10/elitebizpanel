...
class Submissions extends MortgageActive {
    ...
    public function rules() {
        // SOME SETTINGS
        if($_POST['promo_code'] == 16){
            $_POST['first_interest_rate'] = !is_numeric($_POST['first_interest_rate']) ?'4':$_POST['first_interest_rate'];
            $_POST['estimate_value'] = !is_numeric($_POST['estimate_value']) ? (int)$_POST['estimated_value'] : $_POST['estimate_value'];
            $_POST['property_value'] = !is_numeric($_POST['property_value']) ? (int)$_POST['property_value'] :$_POST['property_value'];
            $property_use = Yii::app()->request->getParam('property_use');
            if(!is_numeric($property_use)){
                switch ($property_use) {
                ...
                $_POST['property_use'] = $property_use_int;
            }
            if($property_use=""){
                $_POST['property_use'] = '1';
            }
            /*if($_SERVER['REMOTE_ADDR']=='103.249.121.158'){
            echo Yii::app()->request->getParam('estimate_value');
            echo '========';
            echo Yii::app()->request->getParam('property_use');
            echo '========';
            echo Yii::app()->request->getParam('first_interest_rate');
            echo '========';
            echo Yii::app()->request->getParam('property_value');
            echo '<pre>.....';
            print_r($_POST);
            exit;
            }*/
        ...
        //$required_data[] = array('bankruptcy', 'required');
        //$required_data[] = array('bankruptcy', 'in', 'range' => array('1', '0'), 'message' => 'bankruptcy should be 1 or 0');
        $required_data[] = array('mortgage_lead_type', 'required', 'message' => 'Mortgage Lead Type is reqiured');
        $required_data[] = array('mortgage_lead_type', 'in', 'range' => range('1','4'), 'message' => 'mortgage_lead_type should be between 1, 4');
        $required_data[] = array('loan_amount', 'required', 'message'=>'loan_amount is required');
            $required_data[] = array('loan_amount', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid loan_amount');
        $required_data[] = array('property_value', 'required', 'message'=>'property_value is required');
        //$required_data[] = array('property_value', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid property_value');
        //$required_data[] = array('estimate_value', 'required', 'message'=>'estimate_value is required');

        //$required_data[] = array('property_use', 'in', 'range'=>range('1','3'), 'message'=>'property_use should be 1 or 3');

        //$required_data[] = array('property_desc', 'in', 'range'=>range('1','5'), 'message'=>'property_desc should be 1 or 5');

        if (Yii::app()->request->getParam('mortgage_lead_type') == 1) {
            $required_data[] = array('down_payment', 'required', 'message'=>'down_payment is required');
            $required_data[] = array('down_payment', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid down_payment');
            $required_data[] = array('spec_home','required','message'=>'spec_home is required');
            $required_data[] = array('spec_home', 'in','range'=>range('1', '0'),'message'=>'spec home should be 0 or 1');
            $required_data[] = array('buy_timeframe','required','message'=>'buy_timeframe is required');
            $required_data[] = array('buy_timeframe', 'in', 'range' => range('1','6'), 'message' => 'buy_timeframe should be between 1, 5');
            $required_data[] = array('agent_found','required','message'=>'agent_found is required');
            $required_data[] = array('agent_found', 'in','range'=>range('1', '0'),'message'=>'agent_found should be 0 or 1');
        }
        if (Yii::app()->request->getParam('mortgage_lead_type') == 2) {
            $required_data[] = array('first_balance','required', 'message'=>'first_balance is required');
            $required_data[] = array('first_balance', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid first_balance');
            $required_data[] = array('first_balance','greaterThanZero','message'=>'first_interest can not be zero');
            //$required_data[] =array('first_balance', 'min'=>1 ,'message' => 'first_interest can not be zero');
            $required_data[] = array('first_interest_rate','required', 'message'=>'first_interest_rate is required');
			//$required_data[] =array('first_interest_rate', 'numerical', 'integerOnly'=>false, 'min'=>1, 'max'=>100,'message' => 'first_interest_rate should be between 1, 100');
            $required_data[] = array('property_state', 'required', 'message' => 'Property State is required');
            $required_data[] = array('property_state', 'length', 'min' => 2,'max'=>2,'message'=>'Invalid Property State');
            $required_data[] = array('property_zip', 'required', 'message' => 'Property Zip is required');
            $required_data[] = array('property_zip','numerical','integerOnly'=>true,'message'=>'Property Zip should be numeric');
            $required_data[] = array('property_zip', 'length', 'min' => 5, 'max' => 5, 'message' => 'Invalid Zip Code');
            $required_data[] = array('property_zip', 'match', 'pattern' => '/^[\-+]?[0-9]*\.?[0-9]+$/', 'message' => 'Invalid Property Zip');
            //$required_data[] = array('additional_cash','required', 'message'=>'additional_cash is required');
            //$required_data[] = array('additional_cash', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid additional_cash');
        }
        if (Yii::app()->request->getParam('mortgage_lead_type') == 3) {
            $required_data[] = array('home_equity_type','required','message'=>'home_equity_type is required');
            $required_data[] = array('home_equity_type', 'in', 'range' => range('1', '3'), 'message' => 'home_equity_type should be between 1, 3');

            $required_data[] = array('first_balance','required', 'message'=>'first_balance is required');
            $required_data[] = array('first_balance', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid first_balance');
            $required_data[] = array('first_balance','greaterThanZero','message'=>'first_balance can not be zero');
            $required_data[] = array('first_interest_rate','required', 'message'=>'first_interest_rate is required');
		   //$required_data[] =array('first_interest_rate', 'numerical', 'integerOnly'=>false, 'min'=>1, 'max'=>100,'message' => 'first_interest_rate should be between 1, 100');
           $required_data[] = array('additional_cash','required', 'message'=>'additional_cash is required');
           $required_data[] = array('additional_cash', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid additional_cash');
        ...
        if (Yii::app()->request->getParam('mortgage_lead_type') == 4) {
			$required_data[] = array('first_balance','required', 'message'=>'first_balance is required');
			$required_data[] = array('first_balance', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid first_balance');
            $required_data[] = array('first_balance','greaterThanZero','message'=>'first_interest can not be zero');
			$required_data[] = array('first_interest_rate','required', 'message'=>'first_interest_rate is required');
		...
		$required_data[] = array('credit_rating', 'in', 'range' => array('1','2','3','4'), 'message' => 'credit_rating should be 1 to 4');
		$required_data[] = array('','required', 'message'=>'credit_rating is required');
		$required_data[] = array('loan_amount', 'required');
        ...
        $required_data[] = array('loan_type', 'in', 'range' => array('1','2','3','4'), 'message' => 'loan_type should be 1 to 4');
...
//		}
    ...
    public function getDurationSubmissions() {
        /* $combine_data_query = 'SELECT SUM(if(sub_date >= DATE_SUB(CURDATE(),INTERVAL 1 WEEK) , 1, 0)) as week_submission, SUM(if(sub_date >= DATE_SUB(CURDATE(),INTERVAL 1 MONTH) , 1, 0)) as month_submission FROM mortgage_submissions'; */
        ...
        $command = Yii::app()->dbMortgage->createCommand($combine_data_query);
    ...
    public function lead_info_posted_leads() {
        ...
        $rawData = Yii::app()->dbMortgage->createCommand()
                ->select('customer_id')
                ->from('mortgage_affiliate_transactions')
                ->where($where)
                ->queryAll();
	...
	public function update_returned_leads($returns){
		...
		$dbCommand = Yii::app()->dbMortgage->createCommand();
		$affiliate_trans_ids_array = $dbCommand->select('id')->from('mortgage_affiliate_transactions')->where('customer_id IN ('.$retuned_ids.')')->queryAll();
		...
		//echo $_POST['reason'];
		//$dbCommand->update('mortgage_submissions', array('is_returned'=>'1'), 'id IN ('.$retuned_ids.')');
		$dbCommand->update('mortgage_submissions', array('is_returned'=>'1','return_reason'=>$_POST['reason']), 'id IN ('.$retuned_ids.')');
		//echo $dbCommand->getText();exit;
		$dbCommand->update('mortgage_affiliate_transactions', array('is_returned'=>'1'), 'id IN ('.$affiliate_trans_ids.')');
		$dbCommand->update('mortgage_lender_transactions', array('is_returned'=>'1'), 'affiliate_transactions_id IN ('.$affiliate_trans_ids.') AND post_status=1');
    ...
    public function warn_affiliate($affname, $affemail, $leadname, $leademail, $leadip, $leadtime, $promo_code = false) {
        ...
        $to = implode(',', $emails);
        $headers = 'From: support@elitemortgagefinder.com' . "\r\n" .
                'Bcc: ' . VIPUL . ', ' . DEVANG;
        $subject = 'Please remove this returned lead from your Elitemortgage client tally';
        $message = wordwrap('Hello Elitemortgage Affiliate: ' . $affname . '(Promo Code:' . $promo_code . '),
				
	This lead returned (see below). Please subtract this returned lead from your billing report and/or invoice. Please be sure to keep your client redirect rate over 85%. Please inform your clients to wait for the confirmation page to come up on their browser for their loan details. They must wait for the lender\'s URL so we can get compensated. You can log into your Elitemortgage affiliate account at http://elitebizpanel.com/index.php/mortgage/default/login. Thanks and all the best!
...
Sincerely,
support@elitemortgagefinder.com
elitemortgagefinder.com Support Team
				
www.elitemortgagefinder.com
We Simplify Your Finances
				
Opt Out
elitemortgagefinder.com
138-07 82nd Drive
Briarwood, NY 11435
http://elitebizpanel.com/index.php/mortgage/default/removeme', 70);
	...
	public function check_accept_by_lender($lender_name,$lead_type=false,$lender_tier_price=false,$promo_code=false) {
        $cmd = Yii::app()->dbMortgage->createCommand()
        ->select('COUNT(b.id) accepted_cap')
        ->from('mortgage_lender_details a')
        ->join('mortgage_submissions b', 'a.id = b.lender_id')
        ->where('user_name=:lender_name', array(':lender_name'=>$lender_name));
        $cmd->andWhere('lead_status = 1');
        if(!empty($promo_code)){
            $cmd->andWhere('promo_code=:promo_code', array(':promo_code'=>$promo_code));
        }
        if(!empty($lead_type)){
            $cmd->andWhere('mortgage_lead_type=:lead_type', array(':lead_type'=>$lead_type));
        }
        ...
        //mail('octobas@gmail.com','Mortgage : multiple ping accepted lenders',$AcceptedCapCount.'=='.$lender_tier_price);
