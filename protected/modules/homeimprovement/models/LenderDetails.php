<?php
/**
 * This is the model class for table "homeimprovement_lender_details".
 *
 * The followings are the available columns in table 'homeimprovement_lender_details':
 * @property integer $id
 ...
 */
class LenderDetails extends HomeimprovementActive{
	/**   
	...
	public function tableName(){
		return 'homeimprovement_lender_details';
	}
	...
	public function rules(){
		...
		return array(
		   array('user_name,name','required'),
			...
			array('email,status,first_name,last_name,company_name,phone,submission_cap,minute_submission_cap,hourly_submission_cap,accepted_cap,note','length'),
	...
	public function search(){
		...
		$criteria->compare('minute_submission_cap',$this->minute_submission_cap);
	...
	public function actionPausevendorAjax(){
		...
		$sql = "UPDATE homeimprovement_lender_details SET paused_vendor  = '$val' WHERE id=$id";
	...
	public function check_acceptCap($lender_id) {
		...
		$cmd = Yii::app()->dbHomeimprovement->createCommand()
			->select('submission_cap,accepted_cap,minute_submission_cap,hourly_submission_cap')
			->from('homeimprovement_lender_details')
			->where('id=:lender_id', [':lender_id'=>$lender_id])
			->limit('1');
		$row = $cmd->queryRow();	
		$len_submission_cap = $row['submission_cap'];
		$len_minute_submission_cap = $row['minute_submission_cap'];
		...
		$cmd = Yii::app()->dbHomeimprovement->createCommand()
			...
			->from('homeimprovement_lenders_affiliates_transactions')
			->where($where)
			->limit('1');
		$row = $cmd->queryRow();	
		$submission_cap = $row['total_submission'];
		$accept_cap = $row['total_accept'];
		// MINUTE SUBMISSION CAP
		$where_min_cap = "lender_id = $lender_id AND last_inserted_datetime >= DATE_SUB(NOW(),INTERVAL 1 MINUTE)";
		$cmd = Yii::app()->dbHomeimprovement->createCommand()
			->select('SUM(ping_sent) as minute_submission_cap')
			->from('homeimprovement_lenders_affiliates_transactions')
			->where($where_min_cap)
			->limit('1');
		$row = $cmd->queryRow();	
		$minute_submission_cap = $row['minute_submission_cap'];
        /*if($_SERVER['REMOTE_ADDR']=='81.96.154.57'){
			echo '<br><br>--'. $cmd->getText();
		}*/
		...
		if(empty($InoutDays['id'])){
			...
			return true;
		}else if($len_minute_submission_cap != '-1' && $len_minute_submission_cap <= $minute_submission_cap){
			return false;
	...
	public function check_paused_vendor($lender_id){
		$where = "id= '$lender_id'";
		$array = Yii::app()->dbHomeimprovement->createCommand()
	...
	public function GetAllLenders(){
		$lender_details = Yii::app()->dbHomeimprovement->createCommand()
		->select('id,user_name')
		->from('homeimprovement_lender_details')
		->queryAll();
		$lenders = [];
