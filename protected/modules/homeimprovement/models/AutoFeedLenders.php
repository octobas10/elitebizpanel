...
/**
 * This is the model class for table "homeimprovement_feed_lenders".
 *
 * The followings are the available columns in table 'homeimprovement_feed_lenders':
 * @property integer $id
 ...
 */
class AutoFeedLenders extends HomeimprovementActive
{
	/**
	...
	{
		return 'homeimprovement_feed_lender';
	}
	...
	public function getvendorslist(){
		extract($_REQUEST);
		$update = Yii::app()->dbHomeimprovement->createCommand()->update('homeimprovement_feed_lenders', array('paused_vendor'=>$val),'id=:id',array(':id'=>$id));
		if($val!=""){
			return $val;
		}
	...
	public function check_caps($feed_lender_name,$promo_code = false){
		...
		$feedlenderlist = Yii::app()->dbHomeimprovement->createCommand()
		...
		->from('homeimprovement_feed_lenders')
		...
		if($capdate != @date('Y-m-d')){
			Yii::app()->dbHomeimprovement->createCommand()->update('homeimprovement_feed_lenders', array(
				'capdate'=>@date('Y-m-d'),
		...
		$intervalcount = Yii::app()->dbHomeimprovement->createCommand()
