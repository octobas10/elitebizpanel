...
/**
 * This is the model class for table "homeimprovement_feed_vendor".
 *
 * The followings are the available columns in table 'homeimprovement_feed_vendor':
 * @property integer $id
 ...
 */
class AutoFeedVendor extends HomeimprovementActive {
    /**
     * @return string the associated database table name
     */
    public function tableName(){
        return 'homeimprovement_feed_vendor';
    }
    /**
    ...
    public function listfeedbrowseleads(){
        ...
        $rawData = Yii::app()->dbHomeimprovement->createCommand()->select($string)->from('homeimprovement_feed_submission a_sub')->join('homeimprovement_feed_vendor_transactions a_ft', 'a_sub.id=a_ft.customer_id')->join('homeimprovement_feed_vendor a_afu', 'a_ft.feed_vendor_id  = a_afu.id')->join('homeimprovement_feed_lender_transactions a_lts', 'a_lts.feed_vendor_transactions_id   =  a_ft.id')->where($where)->queryAll();
        return $rawData;
    }
    
    public function listfeedbrowseallleads(){
        $string = "a_sub.*";
        $rawData = Yii::app()->dbHomeimprovement->createCommand()->select($string)->from('homeimprovement_feed_submission a_sub')->join('homeimprovement_feed_vendor_transactions a_ft', 'a_sub.id=a_ft.customer_id')->join('homeimprovement_feed_vendor a_afu', 'a_ft.feed_vendor_id  = a_afu.id')->join('homeimprovement_feed_lender_transactions a_lts', 'a_lts.feed_vendor_transactions_id   =  a_ft.id')->queryAll();
        //echo Yii::app()->dbHomeimprovement->get_query();exit;
        return $rawData;
    }
