<?php
/**
 * This is the model class for table "homeimprovement_affiliate_daily_counts".
 *
 * The followings are the available columns in table 'homeimprovement_affiliate_daily_counts':
 * @property integer $id
 ...
 */
class AffiliateDailyCounts extends HomeimprovementActive
{
	...
	public function tableName(){
		return 'homeimprovement_affiliate_daily_counts';
	}
	/**
	...
	public function update_affiliate_daily_counts($promo_code,$aff_lead_status,$duplicate=false){
		...
		if(Yii::app()->session['ping_type'] == 'post'){
			...
			$query = "INSERT INTO homeimprovement_affiliate_daily_counts VALUES ('".$date."' , $promo_code , 1 , 1 , 1 , 1 , $duplicate , $aff_lead_status) ON DUPLICATE KEY UPDATE post_sent = post_sent + 1 $accept_query $duplicate_query";
		}elseif(Yii::app()->session['ping_type'] == 'directpost'){
			$accept_query = ($aff_lead_status==1) ? ' , post_accepted = post_accepted + 1' : '';
			$duplicate_query = ($duplicate==1) ?  ' , post_duplicate = post_duplicate + 1' : '';
			$query = "INSERT INTO homeimprovement_affiliate_daily_counts VALUES ('".$date."' , $promo_code , 0 , 0 , 0 , 1 , $duplicate , $aff_lead_status) ON DUPLICATE KEY UPDATE post_sent = post_sent + 1 $accept_query $duplicate_query";
		}else{
			$accept_query = ($aff_lead_status==1) ? ' , ping_accepted = ping_accepted + 1' : '';
			$duplicate_query = ($duplicate==1) ?  ' , ping_duplicate = ping_duplicate + 1' : '';
			$query = "INSERT INTO homeimprovement_affiliate_daily_counts VALUES ('".$date."' , $promo_code , 1 , $duplicate, $aff_lead_status , 0 , 0 , 0) ON DUPLICATE KEY UPDATE ping_sent = ping_sent + 1 $accept_query $duplicate_query";
		}
	...
	public function affiliate_pingpost_statistics(){
		...
		$criteria->join = "LEFT JOIN `homeimprovement_affiliate_user` AS b ON a.promo_code = b.id";
	 ...
	 */
	public static function specific_affiliate_pingreport_last_15days($promo_code = null){
		$days = 15;
		$sdate = date('Y-m-d H:i:s',mktime (0,0,0,date('m'),date('d')-$days,date('Y')));
		$edate = date('Y-m-d').' 23:59:59';
		$where[] = $promo_code ? "promo_code = ".$promo_code."" : '0';
		...
		$where = (count($where) > 0) ? ''.implode(' AND ', $where) : '';
		$criteria = new CDbCriteria();
		$criteria->select="promo_code,date,SUM(ping_sent) AS ping_sent,SUM(ping_duplicate) AS ping_duplicate,SUM(ping_accepted) AS ping_accepted";
		$criteria->condition = $where;
		//$criteria->group = 'promo_code';
	...
	public static function specific_affiliate_postreport_last_15days($promo_code=0){
		...
		$result = AffiliateDailyCounts::model()->find($criteria);
		//echo '<pre>============';print_r($criteria);print_r($result);exit;
		if(empty($result)){
			return false;
		}else{
			return $result;
