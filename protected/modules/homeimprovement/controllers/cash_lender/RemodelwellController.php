...
class RemodelwellController extends Controller
{
	...
	public static function getBathroomCategory(){
		$task = Yii::app()->request->getParam('task');
		switch ($task) {
			case '13_1':
				$cat = 'Bathroom Remodel - Bathtub to Shower Conversion';
				break;
			case '13_2':
				$cat = 'Bathroom Remodel - Complete Remodel	';
				break;
			case '13_3':
				$cat = 'Bathroom Remodel - Complete Remodel';
				break;
			...
			case '13_5':
				...
				break;
			case '13_6':
				$cat = 'Bathroom Remodel - Vanity, Fixtures, Sinks, Toilets, other';
				break;
			case '13_7':
				$cat = 'Bathroom Remodel - Vanity, Fixtures, Sinks, Toilets, other';
				break;
			...
			case '13_9':
				...
				break;
			case '13_10':
				$cat = 'Bathroom Remodel - Shower Install or Upgrade';
				break;
			default:
				$cat = 'Bathroom Remodel - Shower Install or Upgrade';
				break;
	...
	public static function getBathroomType(){
		$task = Yii::app()->request->getParam('task');
		switch ($task) {
			case '13_1':
				$type = 'Bath, sinks';
	...
	public static function returnPingData($p1 = false, $p2 = false, $p3 = false, $status = 0, $ping_url = null)
	{
		...
		if ($promo_code != 0) {
			...
			if ($project_type == 13) { //BATHROOM
				$campaign['category'] = self::getBathroomCategory();
				$campaign['remove_walls'] = 'No';
				...
				$campaign['bathroom_type'] = self::getBathroomType();
				$lp_campaign_id = '64232fb2cffdd';
				$lp_campaign_key = 'CFn3fhtWQg2q9PwjDTXR';
			}
			...
			if ($project_type == 33) { //HVAC
				...
				$property_type = rand(1, 3);
				$initial = $property_type > 2 ? 'HVAC Install' : 'HVAC Repair';
				$campaign['category'] = $initial . ' - ' . $air_sub_type;
				$lp_campaign_id = '64233084b5115';
				$lp_campaign_key = '3CwBfYR6QZKx7D2W8mzJ';
			}
			if ($project_type == 42) { //ROOFING
				...
				$lp_campaign_id = '64232f61c9236';
				$lp_campaign_key = '3mBRgD6cYXnT4zWkZVHx';
			}
			...
			if ($project_type == 45) { //SOLAR
				...
				$lp_campaign_id = '64232e9c53b32';
				$lp_campaign_key = 'njdbtqrmfGYpxK7TVzZR';
			}
			if ($project_type == 52) { //WINDOWS
				...
				$campaign['window_number'] = self::$windowNumber[Yii::app()->request->getParam('number_of_windows', '2')];
				$campaign['lp_caller_id'] = $_POST['promo_code'];
				$campaign['comments'] = $_POST['comments'];
				$lp_campaign_id = '64232f0dc67fa';
				$lp_campaign_key = 'v7tP8LMYwdKNnmc9hrJF';
			}
			if ($project_type == 33 || $project_type == 42 || $project_type == 45 || $project_type == 52) {
				...
				$fields = [
					...
					'tcpa_optin' => Yii::app()->request->getParam('tcpa_optin') == 1 ? 'Yes' : 'No',
					'best_time_to_call' => ['Morning', 'Afternoon', 'Evening', 'Any time'][rand(0, 2)],
	...
	public static function returnPingResponse($ping_response)
	{
		...
		if (trim($result[1]) == 'success' || strtolower($result[1]) == 'success') {
			preg_match("/<ping_id>(.*)<\/ping_id>/msui", $ping_response, $pingId);
			preg_match("/<price>(.*)<\/price>/msui", $ping_response, $pingPrice);
			$confirmation_id = $pingId[1];
			$ping_price = $pingPrice[1];
	...
	public static function sendPostData($p1, $p2, $p3, $ping_response, $post_url, $status)
	{
		...
		if ($promo_code != 0) {
			...
			$fields = [
				...
				'email_address' => Yii::app()->request->getParam('email'),
				'ip_address' => Yii::app()->request->getParam('ipaddress'),
				'landing_page_url' => Yii::app()->request->getParam('url', 'https://elitehomeinsurer.com'),
				'tcpa_consent_text' => Yii::app()->request->getParam('tcpa_text'),
				'tcpa_optin' => Yii::app()->request->getParam('tcpa_optin') == 1 ? 'Yes' : 'No',
				'best_time_to_call' => ['Morning', 'Afternoon', 'Evening', 'Any time'][rand(0, 2)],
				'jornaya_lead_id' => Yii::app()->request->getParam('universal_leadid'),
			...
			preg_match("/<result>(.*)<\/result>/msui", $post_response, $result);
			//echo '<pre>';print_r();die();
			if (trim($result[1]) == 'success' or strtolower($result[1]) == 'success') {
				preg_match("/<price>(.*)<\/price>/msui", $post_response, $postPrice);
				$post_status = '1';
				$post_price = isset($postPrice[1]) ? $postPrice[1] : '0.00';
			} else {
				$post_status = '0';
				$post_price = 0;
				$redirect_url = '';
