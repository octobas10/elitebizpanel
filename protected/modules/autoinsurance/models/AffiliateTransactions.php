...
class AffiliateTransactions extends autoInsuranceActive {
	/**
	...
	public function tableName() {
		return 'autoinsurance_affiliate_transactions';
	}
	/**
    ...
    */
	...
	public function browse_aff_transction() {
		...
		$aff_tran_obj=Yii::app()->dbAutoinsurance->createCommand()
			->select('sum(ping_sent) as submissions,sum(post_accepted) as accepted')
			->from('autoinsurance_lenders_affiliates_transactions t')
			->where($where)
			->queryAll();
		return $aff_tran_obj;
	}
	...
	public function acceptLaststate15days(){
		...
		$accept15days = Yii::app()->dbAutoinsurance->createCommand()
	    	->select(array('count(au.id) as cnt1', 'au.state as state'))
		    ->from('autoinsurance_affiliate_transactions af')
		    ->join('autoinsurance_submissions au', 'af.customer_id=au.id')
		    ->where($where)
			->group('state')
			->order('cnt1 DESC')
			->limit(15)
	...
	public function leads_submitted_per_affiliate_last15days(){
		...
		$leads15days = Yii::app()->dbAutoinsurance->createCommand()
	...
	public function log_redirect($affiliate_trans_id,$process) {
    	...
    	/** ---------------------- Invalid Affiliate Transaction Id ---------------------- */
	 	$invalid_tras_id=Yii::app()->dbAutoinsurance->createCommand()
    		->select('id')
    		->from('autoinsurance_affiliate_transactions')
		...
		/** ------------------------- Unaccepted Lead -------------------------------------- */
		$unaccept_tras_id=Yii::app()->dbAutoinsurance->createCommand()
			->select('id')
			->from('autoinsurance_affiliate_transactions')
		...
		/** -------------------------- Duplicate Redirection ------------------------------ */
	 	$dup_redirect_tras_id=Yii::app()->dbAutoinsurance->createCommand()
    		->select('id')
    		->from('autoinsurance_affiliate_transactions')
		...
		$customer_id = $AffiliateTransactions->customer_id;
	 	...
	 	$valid_redirect_tras_id=Yii::app()->dbAutoinsurance->createCommand()->update('autoinsurance_affiliate_transactions', array(
			'redirect'=>'yes',
		), 'id=:id', array(':id'=>$affiliate_trans_id));

	 	/** -------------------- Update submissions table "redirect=yes" ----------------------------------- */
		$submission_redirect=Yii::app()->dbAutoinsurance->createCommand()->update('autoinsurance_submissions', array(
			'redirect'=>'yes',
	...
	public function search_posted_leads(){
		...
		->from('autoinsurance_affiliate_transactions as a')
		->join('autoinsurance_affiliate_user as b','a.promo_code = b.id')
		->where($where)
		->group($groupby)
		->order($order);
	...
	*/
	...
	public function affiliate_stats(){
		...
		/*->select("DATE(date) AS date, COUNT(id) AS ping_sent , SUM(ping_response regexp 'Duplicate') as duplicate_ping , SUM(ping_status=1) as ping_accepted , SUM(post_request!='') as post_sent , SUM(post_status=1) as post_accepted , SUM(is_returned=1) as lead_returned , SUM(IF((post_status=1 AND is_returned IS NULL), vendor_ping_price , 0)) as revenue, SUM(IF((post_status=1 AND is_returned=1), vendor_ping_price , 0)) as returndollar  ")*/
		->from('autoinsurance_affiliate_transactions')
		->where($where)
		->group($groupby)
		->order($orderby);
