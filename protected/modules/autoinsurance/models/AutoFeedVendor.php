...
/**
 * This is the model class for table "autoinsurance_feed_vendor".
 *
 * The followings are the available columns in table 'autoinsurance_feed_vendor':
 * @property integer $id
 ...
 */
class AutoFeedVendor extends autoInsuranceActive {
    /**
     * @return string the associated database table name
     */
    public function tableName(){
        return 'autoinsurance_feed_vendor';
    }
    /**
    ...
    public function listfeedbrowseleads(){
        ...
        $rawData = Yii::app()->dbAutoinsurance->createCommand()->select($string)->from('autoinsurance_feed_submission a_sub')->join('autoinsurance_feed_vendor_transactions a_ft', 'a_sub.id=a_ft.customer_id')->join('autoinsurance_feed_vendor a_afu', 'a_ft.feed_vendor_id  = a_afu.id')->join('autoinsurance_feed_lender_transactions a_lts', 'a_lts.feed_vendor_transactions_id   =  a_ft.id')->where($where)->queryAll();
        return $rawData;
    }
    
    public function listfeedbrowseallleads(){
        $string = "a_sub.*";
        $rawData = Yii::app()->dbAutoinsurance->createCommand()->select($string)->from('autoinsurance_feed_submission a_sub')->join('autoinsurance_feed_vendor_transactions a_ft', 'a_sub.id=a_ft.customer_id')->join('autoinsurance_feed_vendor a_afu', 'a_ft.feed_vendor_id  = a_afu.id')->join('autoinsurance_feed_lender_transactions a_lts', 'a_lts.feed_vendor_transactions_id   =  a_ft.id')->queryAll();
        //echo Yii::app()->dbAutoinsurance->get_query();exit;
        return $rawData;
    }
