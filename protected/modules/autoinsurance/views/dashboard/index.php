...
<div class="row-fluid">
	<div class="span9">
		<?php
			$this->beginWidget('zii.widgets.CPortlet',array('title' => '<span class="icon-picture"></span>Campaign Performance - Auto Insurance'));
			if(is_array($leads)){?>
		...
		<?php
			...
			<tfoot>
				<tr>
					...
					<?php
						foreach($affiliates as $vid => $affiliate_name){
							$total_ping = array_sum(array_column($affiliates_statistics[$vid],'ping_sent'));
						  	$total_accepted_pings = array_sum(array_column($affiliates_statistics[$vid],'ping_accepted'));
							echo '<th class="header" align=right>'.$total_ping.'</th>'.'<th class="header" align=right>'.$total_accepted_pings.'</th>';
						}
						?>
				</tr>
			</tfoot>
				...
				<tr>
					<?php
						foreach($lenders as $lender_id){
							echo '<th style="text-align:center;width:'.(round(90/count($lenders))).'%" colspan="2">'.$Buyers[$lender_id].'</th>';
						}
						?>
				</tr>
				<tr>
				...
				<?php
					...
					foreach($lenders_pingpost_dates as $date){
						...
						foreach($lenders as $lender_id){
							...
							//$total_lender_sale[$lender_id] += $pings;
							//$total_lender_acceptance[$lender_id] += $accepted_pings;
							echo '<td align=right style="">'.$pings.'</td>'.'<td align=right style="">'.$accepted_pings.'</td>';
						}
						echo '</tr>';
					}
		...
		<table id="myTable" class="table table-striped table-hover table-bordered table-condensed">
			<thead>
				...
				<tr>
					<?php
						foreach($lenders as $lender_id){
							echo '<th align=center style="text-align:center;width:'.(round(90/count($lenders))).'%" colspan="2">'.$Buyers[$lender_id].'</th>';
						}
						?>
				</tr>
				<tr>
