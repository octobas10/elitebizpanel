...
class PingprocessController extends Controller
{
    public function actionIndex()
    {
        ...
        if ($reqCount < Yii::app()->params['autoinsurance_ping_cap']) {
            $_POST = $_REQUEST;
            $process = 'inorganic';
            ...
            if ($affiliate_status != 'TestMode') {
                if ($affiliate_cap) {
                    ...
                    if ($promo_code == '1') {
                        ...
                        $dummy_lenders_list = (isset($_POST['lender']) && !empty($_POST['lender'])) ? $_POST['lender'] : implode("','", $dummy_lenders);
                        $lenderlist = $lender_details_model->findAll(array("condition" => "name IN ('" . $dummy_lenders_list . "') AND static_lead_price=0.00", 'order' => 'id DESC'));
                    } else if ($lead_mode == 1) {
                        $lenderlist = $lender_details_model->findAll(array("condition" => "status = 1 AND name != 'DummyLender1' AND name != 'DummyLender2' AND static_lead_price=0.00", 'order' => 'id DESC'));
                    } else {
                    ...
                    foreach ($lenderlist as $lender) {
                        ...
                        if (!$check_submission_accept_cap) 
                        //continue;
                        ...
                        if ($remained_time_limit <= $lender->posting_timelimit || $lender->posting_timelimit != '-1')
                            //continue;
                    ...
                    foreach ($lenderlist as $lender) {
                        ...
                        $arg4 = $lender->status;
                        //$arg4 = 0;
                        $ping_url = ($lender->status == '0' OR $lender->status == '-1') ? $lender->ping_url_test : $lender->ping_url_live;
                    ...
                    //echo '<pre>120::';print_r($ping_responses);echo '</pre>';die();
                    /** ========================END=====================* */
