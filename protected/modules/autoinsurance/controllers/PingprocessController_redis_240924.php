        ...
        if ($affiliate_status != 'TestMode') {
            if ($affiliate_cap) {
                ...
                if ($promo_code == '1') {
                    ...
                    $lenderlist = $lender_details_model->findAll(array("condition" => "status = 1 AND name IN ('" . $dummy_lenders_list . "') AND static_lead_price=0.00", 'order' => 'id DESC'));
                } else if ($lead_mode == 1) {
                ...
                foreach ($lenderlist as $lender) {
					...
					$check_submission_accept_cap = $lender_details_model->check_acceptCap($lender->id); if (!$check_submission_accept_cap) continue;
                ...
                /*if($_SERVER['REMOTE_ADDR'] == '82.36.128.57'){
                    echo '<pre>';print_r($ping_responses);echo '</pre>';die();
                }*/
                /** ========================END=====================* */
