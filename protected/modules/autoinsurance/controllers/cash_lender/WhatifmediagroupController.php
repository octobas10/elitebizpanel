 ...
 */
/**
 * Description of LeadquotesmediaController
 *
 * @author appo
 */
class WhatifmediagroupController {
    ...
    public static function returnPingData($p1 = false, $p2 = false, $p3 = false, $status = 0) {
        $pingData = array();
        $token = !empty($p1) ? $p1 : 'c35d864f745cb4bd98d326ab4a94ffdd438be8c2';
        ...
        $meta = array(
			...
			//'trusted_form_cert_url' => '', //optional
        ...
        $contact = array(
            "zip_code" => Yii::app()->request->getParam('zip'), //required
            "ip_address" => Yii::app()->request->getParam('ipaddress'), //required on POSTF
        );
                ...
                switch (Yii::app()->request->getParam("driver1_incident" . $i . "_type")) {
                    ...
                    case 2:
                        $drivers[0]["accidents"][] = array(
                            ...
                            "description" => self::$accidentDescription[Yii::app()->request->getParam('adriver1_ccident'.$i.'_description')],
...
//        preg_match("/<Confirmation>(.*)<\/Confirmation>/msui", $ping_response, $confirmation_id);
		$token = !empty($parameter1) ? $parameter1 : 'c35d864f745cb4bd98d326ab4a94ffdd438be8c2';
        ...
        $contact = array(
            ...
            "city" => Yii::app()->session['city'], //required on POST
            "state" => Yii::app()->session['state'], //required on POST
		...
		$converage_type = Yii::app()->request->getParam('coverage_type', '1');
        if (Yii::app()->request->getParam('insurance_policy', '0')) {
            $fields["data"]["current_policy"] = array(
                "insurance_company" => $InsuranceComapany['insurance_company_name'],
                "expiration_date" => date('Y-m-d', strtotime(Yii::app()->request->getParam('insurance_expiration_date'))),
                "insured_since" => date('Y-m-d', strtotime(Yii::app()->request->getParam('insurance_start_date'))),
