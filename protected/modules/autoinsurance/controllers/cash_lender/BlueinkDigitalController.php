...
/**
 * Description of BlueinkDigitalController
 *
 * @author appo
 */
class BlueinkDigitalController
{
    public static $not_allowed = ['insurancechiefs.com', 'protect.com'];

    public static $companyListFromBuyer = ['21st Century Insurance', 'AAA', 'AIG', 'Allstate', 'American Family', 'American National Property and Casualty', 'Brightway Insurance', 'Country Financial', 'Erie', 'Farm Bureau', 'Farmers', 'GEICO', 'GMAC', 'Hartford', 'Infinity', 'Liberty Mutual', 'Mercury', 'MetLife', 'Mutual of Omaha', 'Nationwide', 'Progressive', 'Safeco', 'Shelter Insurance Co.', 'State Farm', 'The Hartford', 'Travelers', 'Unitrin Inc.', 'USAA'];
    ...
    public static $ticketDescription = [
        1 => 'Careless driving',
    ...
    public static function returnPingData($p1 = false, $p2 = false, $p3 = false, $status = 0)
    {
        ...
        $InsuranceComapany = $submission_model->getMatchingCompanyFromPublisher(self::$companyListFromBuyer, self::$companyListFromBuyer[0]);
        ...
        $url = trim($url) <> "" ? $url : 'https://eliteinsurers.com';
        $meta = [
            ...
            'landing_page_url' => $url, //required
        ];
        $contact = [
            "zip_code" => $zip_code, //required
            "ip_address" => Yii::app()->request->getParam('ipaddress', '10.0.0.1'), //required on POSTF
        ];
        ...
        $drivers1 = [
            '0' => [
                "birth_date" => date('m/d/Y', strtotime(Yii::app()->request->getParam('driver1_dob'))),
                ...
                "occupancy_date" => date("m/d/Y"),
                "relationship" => 'Self',
                "license_status" => 'Active',
                "license_state" => $city_state['state'],
                "requires_sr22" => Yii::app()->request->getParam('driver1_required_SR22', '0') == '0' ? 'false' : 'true',
                "bankruptcy" => Yii::app()->request->getParam('bankruptcy', '0') == '0' ? 'false' : 'true',
        ...
        if (Yii::app()->request->getParam('driver2_gender')) {
            $drivers2 = [
                '1' => [
                    "birth_date" => date('m/d/Y', strtotime(Yii::app()->request->getParam('driver2_dob'))),
                    ...
                    "requires_sr22" => Yii::app()->request->getParam('driver2_required_SR22', '0') == '0' ? 'false' : 'true',
                    'education' => self::$education[Yii::app()->request->getParam('driver2_education', '1')],
                    'occupation' => $driver2_occupation,
                    'residence_type' => (Yii::app()->request->getParam('is_rented') == 'rent') ? 'Rent' : 'Own',
                    //'months_at_residence' => (($years_at_residence * 12) + $months_at_residence),
                ]
            ];
        ...
        if (Yii::app()->request->getParam('driver1_hasTAVCs')) {
            for ($i = 1; $i <= Yii::app()->request->getParam('driver1_num_of_incidents'); $i++) {
                switch (Yii::app()->request->getParam("driver1_incident" . $i . "_type")) {
                    case 1:
                        $drivers[0]["tickets"][] = [
                            "ticket_date" => date('m/d/Y', strtotime(Yii::app()->request->getParam('driver1_incident' . $i . '_date'))),
                            "description" => self::$ticketDescription[Yii::app()->request->getParam('driver1_ticket' . $i . '_description')],
                        ];
                        break;
                    case 2:
                        $drivers[0]["accidents"][] = [
                            "accident_date" => date('m/d/Y', strtotime(Yii::app()->request->getParam('driver1_incident' . $i . '_date'))),
                        ...
                        break;
                    case 3:
                        $drivers[0]["major_violations"][] = [
                            "violation_date" => date('m/d/Y', strtotime(Yii::app()->request->getParam('driver1_incident' . $i . '_date'))),
                    ...
                    case 4:
                        $drivers[0]["claims"][] = [
                            "claim_date" => date('m/d/Y', strtotime(Yii::app()->request->getParam('driver1_incident' . $i . '_date'))),
        ...
        $vehicles1 = [
            "0" => [
                ...
                "submodel" => Yii::app()->request->getParam('vehicle1_submodel', '1'),
                "vin" => Yii::app()->request->getParam('vehicle1_vin', "1ACBC535*B*******"),
        ...
        if (Yii::app()->request->getParam('vehicle2_year')) {
            $vehicles2 = [
                "1" => [
                    ...
                    "submodel" => Yii::app()->request->getParam('vehicle2_submodel', '1'),
        ...
        if (Yii::app()->request->getParam('insurance_policy', '1')) {
            $fields["data"]["current_policy"] = [
                "insurance_company" => $InsuranceComapany,
        ...
        if ($purchase == true) {
            $pingData['ping_request'] = json_encode($fields, JSON_PRETTY_PRINT);
            $pingData['header'] = [
    ...
    public static function sendPostData($parameter1, $parameter2, $parameter3, $ping_response, $post_url, $status)
    {
        ...
        $contact = [
            "first_name" => Yii::app()->request->getParam('driver1_first_name'), //required on POST
            "last_name" => Yii::app()->request->getParam('driver1_last_name'), //required on POST
            "email" => Yii::app()->request->getParam('email'), //required on POST
            "phone" => Yii::app()->request->getParam('phone'), //required on POST
            "address" => Yii::app()->request->getParam('address'), //required on POST
            "city" => Yii::app()->session['city'] ? Yii::app()->session['city'] : $city_state['city'],
            "state" => Yii::app()->session['state'] ? Yii::app()->session['state'] : $license_state,
        ...
        if (Yii::app()->request->getParam('driver2_first_name')) {
            $drivers2 = [
                '1' => [
                    "first_name" => Yii::app()->request->getParam('driver2_first_name'), //required on POST
                    "last_name" => Yii::app()->request->getParam('driver2_last_name'), //required on POST
                    "birth_date" => date('m/d/Y', strtotime(Yii::app()->request->getParam('driver2_dob'))),
                    "marital_status" => self::$maritalStatus[Yii::app()->request->getParam('driver2_marital_status', '1')],
                    "license_status" => 'Active',
                    "relationship" => 'Spouse',
        ...
        $authToken = $parameter1 ? $parameter1 : '70bf76af-f659-4bf2-9774-153fd322ed7c';
        $header = [
            "authorization: Token $authToken",
            "content-type: application/json",
        ];
