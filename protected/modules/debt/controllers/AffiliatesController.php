...
class AffiliatesController extends Controller{
	...
	public function actionSupression_list(){
		...
		if(Yii::app()->request->getParam('email_suprression_list') || Yii::app()->request->getParam('phone_supression_list')) {
			$table = 'debt_email_supression_list';
			$fields = 'email';
			...
			if(Yii::app()->request->getParam('email_suprression_list')){
				...
				$table = 'debt_phone_supression_list';
				$fields = 'phone';
				$filename = 'phone_suprression_list-'.date('Y-m-d');
			}
			
			$query_data = Yii::app()->dbDebt->createCommand()
			->select($fields)
			->from($table)
			->queryAll();
			...
			if (!empty($emails)){
				...
				foreach ($name_emails as $email) {
					...
					if (@eregi("^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$", $email)) {
						$email_supression = Yii::app()->dbDebt->createCommand()->select('email')
						->from('debt_email_supression_list')
						->where("`email`='". $email."'")
						->query();
						if($email_supression->rowCount==0){
							Yii::app()->dbDebt->createCommand()->insert('debt_email_supression_list',array('email'=>$email));
			...
			if (!empty($phones)){
				...
				foreach ($name_phones as $phone) {
					...
					//if(@eregi("/^[\d]{10}$/", $phone)) {
						...
						$phone_supression = Yii::app()->dbDebt->createCommand()->select('phone')
						->from('debt_phone_supression_list')
						->where("phone=". $phone)
						->query();
						if($phone_supression->rowCount==0){
							Yii::app()->dbDebt->createCommand()->insert('debt_phone_supression_list',array('phone'=>$phone));
							/*$fileName = '../suppressionlist.txt';
							 $file=fopen($fileName,"a") or die("$fileName does not exist!");
							fwrite($file, $email."\n");*/
						}
	...
	public function actionCreatives(){
		...
		if(Yii::app()->request->getParam('upload')){
			...
			if($uploadOk == 0){
				...
				if (move_uploaded_file($file_tmp_name, $target_dir.$save_name)) {
					$res = Yii::app()->dbDebt->createCommand()->insert($table='promotional_creatives',$columns=array('private_label'=>$private_label,'promotional_text'=>$promotional_text,'image_name'=>$save_name,'promo_code'=>$promo_code,'c_date'=>date('Y-m-d H:i:s')));
		...
		if(Yii::app()->request->getParam('remove_id')!=''){
			...
			$res = Yii::app()->dbDebt->createCommand()->delete($table='promotional_creatives',$conditions=$condition,$params=array());
			$this->redirect('creatives');
		}
		
		$creatives = Yii::app()->dbDebt->createCommand()->select('id,private_label,promotional_text,image_name,promo_code,c_date')
	...
	public function actionEmailcreatives(){
		...
		if(Yii::app()->request->getParam('upload')){
			...
			if($uploadOk == 0){
				...
				$save_name =  $filename_without_extension.'_'.time().'.'.$imageFileType;
				if (move_uploaded_file($file_tmp_name, $target_dir.$save_name)) {
					$res = Yii::app()->dbDebt->createCommand()->insert($table='email_creatives',$columns=array('image_name'=>$save_name,'promo_code'=>$promo_code,'c_date'=>date('Y-m-d H:i:s')));
		...
		if(Yii::app()->request->getParam('add_subjectlines')){
			extract($_POST);
			$res = Yii::app()->dbDebt->createCommand()->insert($table='email_creatives_subject_lines',$columns=array('subject_lines'=>$email_creatives_subject_line,'promo_code'=>$promo_code,'c_date'=>date('Y-m-d H:i:s')));
			$this->redirect('emailcreatives');
		}
		if(Yii::app()->request->getParam('add_fromlines')){
			extract($_POST);
			$res = Yii::app()->dbDebt->createCommand()->insert($table='email_creatives_from_lines',$columns=array('from_lines'=>$email_creatives_from_line,'promo_code'=>$promo_code,'c_date'=>date('Y-m-d H:i:s')));
			$this->redirect('emailcreatives');
		}
		if(Yii::app()->request->getParam('remove_id')!=''){
			extract($_POST);
			$condition = 'id='.$remove_id;
			$res = Yii::app()->dbDebt->createCommand()->delete($table='email_creatives',$conditions=$condition,$params=array());
			$this->redirect('creatives');
		}
	
		$creatives = Yii::app()->dbDebt->createCommand()->select('id,image_name,promo_code,c_date')
		->from('email_creatives')
		->order('c_date desc')
		->queryAll();
		
		$email_creatives_subject_lines = Yii::app()->dbDebt->createCommand()->select('id,subject_lines,promo_code,c_date')
		->from('email_creatives_subject_lines')
		->order('c_date desc')
		->queryAll();
		
		$email_creatives_from_lines = Yii::app()->dbDebt->createCommand()->select('id,from_lines,promo_code,c_date')
	...
	public function actionGetaffstatus(){
		...
		if($promo_code){
			...
			if($row['status'] =='0' && $row['is_inorganic'] =='0'){
				...
				if($row['pixel_type'] == 1){
					mail('octobas@gmail.com,vicd@rcn.com','s2spixel debt',$pixel_code.'-->Subid : '.$sub_id.'-->Subid2 :'.$sub_id2);
