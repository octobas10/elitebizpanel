...
/**
 * This is the model class for table "debt_affiliate_daily_counts".
 *
 * The followings are the available columns in table 'debt_affiliate_daily_counts':
 * @property integer $id
 ...
 */
class AffiliateDailyCounts extends DebtActive
{
	...
	{
		return 'debt_affiliate_daily_counts';
	}
	...
	public function update_affiliate_daily_counts($promo_code,$aff_lead_status,$duplicate=false){
		...
		if(Yii::app()->session['ping_type'] == 'post'){
			...
			$query = "INSERT INTO debt_affiliate_daily_counts VALUES ('".$date."' , $promo_code , 1 , 1 , 1 , 1 , $duplicate , $aff_lead_status) ON DUPLICATE KEY UPDATE post_sent = post_sent + 1 $accept_query $duplicate_query";
		}elseif(Yii::app()->session['ping_type'] == 'directpost'){
			$accept_query = ($aff_lead_status==1) ? ' , post_accepted = post_accepted + 1' : '';
			$duplicate_query = ($duplicate==1) ?  ' , post_duplicate = post_duplicate + 1' : '';
			$query = "INSERT INTO debt_affiliate_daily_counts VALUES ('".$date."' , $promo_code , 0 , 0 , 0 , 1 , $duplicate , $aff_lead_status) ON DUPLICATE KEY UPDATE post_sent = post_sent + 1 $accept_query $duplicate_query";
		}elseif(Yii::app()->session['ping_type'] == 'directping'){
			$accept_query = ($aff_lead_status==1) ? ' , ping_accepted = ping_accepted + 1' : '';
			$duplicate_query = ($duplicate==1) ?  ' , ping_duplicate = ping_duplicate + 1' : '';
			$query = "INSERT INTO debt_affiliate_daily_counts VALUES ('".$date."' , $promo_code , 1 , $duplicate, $aff_lead_status , 0 , 0 , 0) ON DUPLICATE KEY UPDATE ping_sent = ping_sent + 1 $accept_query $duplicate_query";
		}else{
			$accept_query = ($aff_lead_status==1) ? ' , ping_accepted = ping_accepted + 1' : '';
			$duplicate_query = ($duplicate==1) ?  ' , ping_duplicate = ping_duplicate + 1' : '';
			$query = "INSERT INTO debt_affiliate_daily_counts VALUES ('".$date."' , $promo_code , 1 , $duplicate, $aff_lead_status , 0 , 0 , 0) ON DUPLICATE KEY UPDATE ping_sent = ping_sent + 1 $accept_query $duplicate_query";
	...
	public function affiliate_pingpost_statistics(){
		...
		$criteria->join = "LEFT JOIN `debt_affiliate_user` AS b ON a.promo_code = b.id";
