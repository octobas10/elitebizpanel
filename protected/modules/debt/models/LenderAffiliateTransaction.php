<?php
class LenderAffiliateTransaction extends DebtActive{
	...
	public function tableName(){
		return 'debt_lenders_affiliates_transactions';
	}
	/**
	...
	public function check_lender_affiliate_cap($promo_code,$lender_id){
		...
		$vendor_lender_cap = Yii::app()->dbDebt->createCommand()
			->select('cap,intervals')
			->from('debt_lender_affiliate_settings')
			->where($where)
			->queryAll();
		...
		$submission_of_lender = Yii::app()->dbDebt->createCommand()
		...
		$time_at_last_lead_sent = Yii::app()->dbDebt->createCommand()
	...
	public function lender_ping_status_update($lender_id,$promo_code,$ping_status,$price){
		...
		$sql1="INSERT INTO debt_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),1,$ping_status,0,0,now()) on duplicate key update `ping_sent`=`ping_sent`+1 $accept_query, last_inserted_datetime=NOW()";
	...
	public function lender_post_status_update($lender_id,$promo_code,$post_status,$price,$static_price_direct_post=false){
		...
		if($static_price_direct_post==false){
			$sql1="INSERT INTO debt_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),1,1,1,$post_status,now()) on duplicate key update `post_sent`=`post_sent`+1 $accept_query, last_inserted_datetime=NOW()";
		}elseif($static_price_direct_post==true){
			$sql1="INSERT INTO debt_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),0,0,1,$post_status,now()) on duplicate key update `post_sent`=`post_sent`+1 $accept_query, last_inserted_datetime=NOW()";
		}
		$command=$connection->createCommand($sql1);
		$dataReader=$command->query();
	}
