<?php
class FeedLenderTransactions extends DebtActive {
    /**
    ...
    public function tableName() {
        return 'debt_feed_lender_transactions';
    }
    /**
...
//  		echo '<pre>'.$where.'</pre>';exit;
		...
		return $rawData = Yii::app()->dbDebt->createCommand()
	    ->select('id,feed_lender_name,date,request,full_response,response,')
	    ->from('debt_feed_lender_transactions')
		->where($where)
		->order($order)
	    ->queryAll();
	}
