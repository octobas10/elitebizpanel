<?php
/**
 * This is the model class for table "debt_lender_transactions".
 *
 * The followings are the available columns in table 'debt_lender_transactions':
 * @property integer $id
 ...
 */
class LenderTransactions extends DebtActive{
	/**
	...
	public function tableName(){
		return 'debt_lender_transactions';
	}
	/**
	 *
	...
	public function exit_url($affiliate_trans_id){
		...
		$exiturl = Yii::app()->dbDebt->createCommand()
		->select('exit_url')
		->from('debt_lender_transactions')
		->where($where)
		->queryScalar();
	...
	public function get_Lender_Pigned_With_This_Ping_Id($affiliate_trans_id){
		...
		$dbCommand = Yii::app()->dbDebt->createCommand()->setFetchMode(PDO::FETCH_OBJ)
		...
		->from('debt_lender_transactions a')
		...
		//mail('octobas@gmail.com','debt : multiple ping accepted lenders',$qry);
		return $dataReader;
	}
	/**
	...
	public function lender_reports(){
		...
		$rawData1 = Yii::app()->dbDebt->createCommand()
		...
		->from('debt_lender_transactions as a')
		->where($where)
		->group($groupby)
		->order($order)
		->queryAll();
