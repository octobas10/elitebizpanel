...
class AffiliateTransactions extends DebtActive
{
	/**
	...
	{
		return 'debt_affiliate_transactions';
	}
	/**
	...
	public function browse_aff_transction()
	{
		...
		$aff_tran_obj = Yii::app()->dbDebt->createCommand()
			->select('sum(ping_sent) as submissions,sum(post_accepted) as accepted')
			->from('debt_lenders_affiliates_transactions t')
			->where($where)
			->queryAll();
		return $aff_tran_obj;
	}
	...
	public function acceptLaststate15days()
	{
		...
		$accept15days = Yii::app()->dbDebt->createCommand()
			->select(array('count(au.id) as cnt1', 'au.state as state'))
			->from('debt_affiliate_transactions af')
			->join('debt_submissions au', 'af.customer_id=au.id')
			->where($where)
			->group('state')
			->order('cnt1 DESC')
			->limit(15)
	...
	public function leads_submitted_per_affiliate_last15days()
	{
		...
		$leads15days = Yii::app()->dbDebt->createCommand()
	...
	public function log_redirect($affiliate_trans_id, $process)
	{
		...
		$invalid_tras_id = Yii::app()->dbDebt->createCommand()
			->select('id')
			->from('debt_affiliate_transactions')
		...
		/** ------------------------- Unaccepted Lead -------------------------------------- */
		$unaccept_tras_id = Yii::app()->dbDebt->createCommand()
			->select('id')
			->from('debt_affiliate_transactions')
		...
		$dup_redirect_tras_id = Yii::app()->dbDebt->createCommand()
			->select('id')
			->from('debt_affiliate_transactions')
			->where(array('and', 'id=' . $affiliate_trans_id, 'redirect="1"'))
			->query();
		...
		$AffiliateTransactions = AffiliateTransactions::model()->findByPK($affiliate_trans_id);
		/** ----------- Update affiliate transactions table "redirect=1" ------------------------ */
		Yii::app()->dbDebt->createCommand()->update('debt_affiliate_transactions', array(
			'redirect' => '1',
		), 'id=:id', array(':id' => $affiliate_trans_id));

		/** ------------ Update submissions table "redirect=1" ----------------------------------- */
		Yii::app()->dbDebt->createCommand()->update('debt_submissions', array(
			'redirect' => '1',
			'redirect_ip' => $_SERVER['REMOTE_ADDR'],
		), 'id=:id', array(':id' => $customer_id));
		Yii::app()->dbDebt->createCommand()->update('debt_lender_transactions', array(
			'redirect' => '1',
		), 'customer_id=:customer_id AND post_status=1', array(':customer_id' => $customer_id));
		if ($process == 'organic') {
	...
	public function search_posted_leads()
	{
		...
		$dbCommand = Yii::app()->dbDebt->createCommand()
			...
			->from('debt_affiliate_transactions as a')
			->join('debt_affiliate_user as b', 'a.promo_code = b.id')
			->where($where)
			->group($groupby)
			->order($order);
