...
/**
 * This is the model class for table "debt_feed_lenders".
 *
 * The followings are the available columns in table 'debt_feed_lenders':
 * @property integer $id
 ...
 */
class AutoFeedLenders extends DebtActive
{
	/**
	...
	public function getvendorslist(){
		extract($_REQUEST);
		$update = Yii::app()->dbDebt->createCommand()->update('debt_feed_lenders', array('paused_vendor'=>$val),'id=:id',array(':id'=>$id));
		if($val!=""){
			return $val;
		}
	...
	public function check_caps($feed_lender_name,$promo_code = false){
		...
		$feedlenderlist = Yii::app()->dbDebt->createCommand()
		...
		if($capdate != @date('Y-m-d')){
			Yii::app()->dbDebt->createCommand()->update('debt_feed_lenders', array(
				'capdate'=>@date('Y-m-d'),
		...
		$intervalcount = Yii::app()->dbDebt->createCommand()
