...
<div class="row-fluid">
	<div class="span12">
		<table id="posts" class="table table-striped table-hover table-bordered table-condensed">
			...
			<tbody>
				<?php $connection=Yii::app()->dbDebt; foreach ($posts as $row) { ?>
				<tr class="post">
					<td>
						<?php echo $row['date']; ?>
					</td>
					...
					<td>
						<?php $command=$connection->createCommand('SELECT * FROM debt_lender_transactions WHERE affiliate_transactions_id ='.$row['id']); $res = $command->queryAll(); foreach ($res as $row) { ?>
						<table class="table table-striped table-hover table-bordered table-condensed">
							<tr>
								<td>
									<?php echo $row[ 'name']; ?>
