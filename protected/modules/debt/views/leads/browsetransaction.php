...
<tr>
	...
	<td style="width:150px;"><b>Promo code :</b><br>
	...
	<td style="width:155px;"><b>Post Response :</b><br>
	<?php echo CHtml::radioButtonList('post_status', ''.$post_status.'', 
			array(''=>'ANYTHING', '1' => 'ACCEPTED', '0' => 'REJECTED','-1' => 'ERROR'), 
			array('labelOptions'=>array('style'=>'display:inline')));
	?>
	<div id="lender_id" style="display:none;padding-top:10px;">
	<?php
		$lender_id = Yii::app()->getRequest()->getParam('lender_id');
		echo Chtml::listBox('lender_id', ''.$lender_id. '',
				CHtml::listData(LenderDetails::model()->findAll(),'id','name'),
				array('empty'=>'-All Lenders-','style'=>'width:150px'));
	?>
	</div>
	</td>
	<td style="width:130px;"><b>Fields :</b><br>
	<?php echo CHtml::radioButtonList('field',''.$field.'', 
			array('email'=>'Email','first_name' => 'First Name', 'last_name' => 'Last Name', 'ipaddress' => 'IP Address', 'universal_leadid' => 'Jornaya'),	
			array('labelOptions' => array('style'=>'display:inline')));
	?>
	</td>	
...
<?php
 ...
 foreach ($posts as $aff_trans) {?>
	
	<tr class="post">
		<!--<td><?php echo $aff_trans['id']; ?></td>-->
		<td><?php echo $aff_trans['date']; ?></td>
		<td><?php $model = AffiliateUser::model()->findByPk($aff_trans['promo_code']); 
		echo $model->user_name.' ('.$aff_trans['promo_code'].')'; ?></td>
		<td>
		...
		<td><?php
			if(!empty($aff_trans->lender_trnas)){
			foreach ($aff_trans->lender_trnas as $len_trans) { ?>
			<table class="table table-striped table-hover table-bordered table-condensed">
				<tr>
					...
					<td>
						...
						<?php }?>
						<p>
						<?php echo "Redirected :  ".(isset($len_trans['redirect']) && $len_trans['redirect']=='1' ? 'Yes' : 'No'); ?>
						</p>
					</td>
					<td>
						<p><b>Ping Price</b></p>
...
$(document).ready(function() {
    ...
    if('<?php echo $post_status;?>'=="1"){ $('#lender_id').show();}else{$('#lender_id').hide();}
