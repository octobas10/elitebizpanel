<?php
class FeedLenderTransactions extends BusinessloansActive {
    /**
    ...
    public function tableName() {
        return 'businessloans_feed_lender_transactions';
    }
    /**
...
//  		echo '<pre>'.$where.'</pre>';exit;
		...
		return $rawData = Yii::app()->dbBusinessLoans->createCommand()
	    ->select('id,feed_lender_name,date,request,full_response,response,')
	    ->from('businessloans_feed_lender_transactions')
		->where($where)
		->order($order)
	    ->queryAll();
	}
