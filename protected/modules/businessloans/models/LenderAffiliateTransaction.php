<?php
class LenderAffiliateTransaction extends BusinessloansActive{
	...
	public function tableName(){
		return 'businessloans_lenders_affiliates_transactions';
	}
	/**
	...
	public function check_lender_affiliate_cap($promo_code,$lender_id){
		...
		$vendor_lender_cap = Yii::app()->dbBusinessLoans->createCommand()
			->select('cap,intervals')
			->from('businessloans_lender_affiliate_settings')
			->where($where)
			->queryAll();
		...
		$submission_of_lender = Yii::app()->dbBusinessLoans->createCommand()
			->select('SUM(ping_sent) as todays_submission')
			->from('businessloans_lenders_affiliates_transactions')
			->where($where1)
			->queryScalar();
		/** Calculation of time interval when last lead sent */
		$time_at_last_lead_sent = Yii::app()->dbBusinessLoans->createCommand()
	...
	public function lender_ping_status_update($lender_id,$promo_code,$ping_status,$price){
		...
		$sql1="INSERT INTO businessloans_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),1,$ping_status,0,0,now()) on duplicate key update `ping_sent`=`ping_sent`+1 $accept_query, last_inserted_datetime=NOW()";
	...
	public function lender_post_status_update($lender_id,$promo_code,$post_status,$price,$static_price_direct_post=false){
		...
		if($static_price_direct_post==false){
			$sql1="INSERT INTO businessloans_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),1,1,1,$post_status,now()) on duplicate key update `post_sent`=`post_sent`+1 $accept_query, last_inserted_datetime=NOW()";
		}elseif($static_price_direct_post==true){
			$sql1="INSERT INTO businessloans_lenders_affiliates_transactions VALUES($lender_id,$promo_code,$price,curdate(),0,0,1,$post_status,now()) on duplicate key update `post_sent`=`post_sent`+1 $accept_query, last_inserted_datetime=NOW()";
		}
		$command=$connection->createCommand($sql1);
		$dataReader=$command->query();
	}
	...
	public function lender_details_from_affiliate_tranaction_id($affiliate_trans_id){
		...
		$lender_details = Yii::app()->dbBusinessLoans->createCommand()
	 ...
	 */
	public static function specific_lender_ping_report_last_15days($lender_id = ''){
	 ...
	 */
	public static function specific_lender_post_report_last_15days($lender_id = ''){
		$sdate = date('Y-m-d',mktime(0,0,0,date('m'),date('d')-15,date('Y')));
		$edate = date('Y-m-d');
		$criteria = new CDbCriteria();
