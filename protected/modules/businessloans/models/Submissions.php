<?php
class Submissions extends BusinessloansActive {
    public $dob_month, $dob_day, $dob_year, $new_car;
    public static function model($className = __CLASS__) {
        return parent::model($className);
    }
    public function tableName() {
        return 'businessloans_submissions';
    }
    ...
    public function rules() {
        ...
        $required_data[] = array('trustedformcerturl', 'required', 'message' => 'Trusted Form Url is required');
        ...
        //$required_data[] = array('debt_amount', 'required', 'message'=>'Debt Amount is required');
        //$required_data[] = array('debt_amount', 'match', 'pattern' => '/^([0-9]*)$/', 'message' => 'Invalid Debt Amount');
        ...
        //$required_data[] = array('business_name', 'required', 'message'=>'Business Name is required');
        //$required_data[] = array('how_long_in_business','required','message'=>'How Long have you in business required');
		//$required_data[] = array('how_much_want_borrow', 'required','message'=>'How much do you want to borrow required');
    ...
    public function checkDuplicate($data) {
        $dbCommand = Yii::app()->dbBusinessLoans->createCommand()
                ->select('id')
                ->from('businessloans_submissions')
    ...
    public function checkPingDuplicate($data) {
        $dbCommand = Yii::app()->dbBusinessLoans->createCommand()
                ->select('dup_days')
                ->from('businessloans_affiliate_user')
    ...
    public function afterSave() {
        $id = Yii::app()->dbBusinessLoans->getLastInsertId();
        if ($id != 0)
...
//		}
        ...
        $rawData = Yii::app()->dbBusinessLoans->createCommand()
                ->select($fields)
                ->from("businessloans_submissions a_sub")
                ->where($where)
                ->order($orderby)
                ->queryAll();
        return $rawData;
    ...
    public function getDurationSubmissions() {
        /* $combine_data_query = 'SELECT SUM(if(sub_date >= DATE_SUB(CURDATE(),INTERVAL 1 WEEK) , 1, 0)) as week_submission, SUM(if(sub_date >= DATE_SUB(CURDATE(),INTERVAL 1 MONTH) , 1, 0)) as month_submission FROM businessloans_submissions'; */
        ...
        $command = Yii::app()->dbBusinessLoans->createCommand($combine_data_query);
    ...
    public function affiliate_revenue_statistics(){
        ...
        $dbCommand = Yii::app()->dbBusinessLoans->createCommand()
        ->select($fields)
        ->from("businessloans_submissions as A")
        ->where($where)
        ->order($orderby)
        ->group($groupby);
    ...
    public function lender_revenue_statistics(){
        ...
        ->select($fields)
        ->from("businessloans_submissions as A")
        ->join('businessloans_lender_details B','A.lender_id = B.id')
        ->where($where)
        ->order($orderby)
        ->group($groupby);
	...
	public function update_returned_leads($returns){
		...
		$dbCommand = Yii::app()->dbBusinessLoans->createCommand();
		$affiliate_trans_ids_array = $dbCommand->select('id')->from('businessloans_affiliate_transactions')->where('customer_id IN ('.$retuned_ids.')')->queryAll();
		...
		//echo $_POST['reason'];
		//$dbCommand->update('businessloans_submissions', array('is_returned'=>'1'), 'id IN ('.$retuned_ids.')');
		$dbCommand->update('businessloans_submissions', array('is_returned'=>'1','return_reason'=>$_POST['reason']), 'id IN ('.$retuned_ids.')');
		//echo $dbCommand->getText();exit;
		$dbCommand->update('businessloans_affiliate_transactions', array('is_returned'=>'1'), 'id IN ('.$affiliate_trans_ids.')');
		$dbCommand->update('businessloans_lender_transactions', array('is_returned'=>'1'), 'affiliate_transactions_id IN ('.$affiliate_trans_ids.') AND post_status=1');
    ...
    public function warn_affiliate($affname, $affemail, $leadname, $leademail, $leadip, $leadtime, $promo_code = false) {
        ...
        $to = implode(',', $emails);
        $headers = 'From: support@elitebusinessloans.com' . "\r\n" .
                'Bcc: ' . VIPUL . ', ' . DEVANG;
        $subject = 'Please remove this returned lead from your elitebusinessloanscleaners client tally';
        $message = wordwrap('Hello elitebusinessloanscleaners Affiliate: ' . $affname . '(Promo Code:' . $promo_code . '),
				
	This lead returned (see below). Please subtract this returned lead from your billing report and/or invoice. Please be sure to keep your client redirect rate over 85%. Please inform your clients to wait for the confirmation page to come up on their browser for their loan details. They must wait for the lender\'s URL so we can get compensated. You can log into your elitebusinessloanscleaners affiliate account at http://elitebizpanel.com/index.php/businessloans/default/login. Thanks and all the best!
...
Sincerely,
support@elitebusinessloans.com
elitebusinessloans.com Support Team
				
www.elitebusinessloans.com
We Simplify Your Finances
				
Opt Out
elitebusinessloans.com
138-07 82nd Drive
Briarwood, NY 11435
http://elitebizpanel.com/index.php/businessloans/default/removeme', 70);
	...
	public function check_accept_by_lender($lender_name,$lead_type=false,$lender_tier_price=false) {
        $cmd = Yii::app()->dbBusinessLoans->createCommand()
        ->select('COUNT(b.id) accepted_cap')
        ->from('businessloans_lender_details a')
        ->join('businessloans_submissions b', 'a.id = b.lender_id')
        ...
        //mail('octobas@gmail.com','businessloans : multiple ping accepted lenders',$AcceptedCapCount.'=='.$lender_tier_price);
