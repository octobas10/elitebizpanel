...
/**
 * This is the model class for table "businessloans_feed_vendor".
 *
 * The followings are the available columns in table 'businessloans_feed_vendor':
 * @property integer $id
 ...
 */
class AutoFeedVendor extends BusinessloansActive {
    /**
     * @return string the associated database table name
     */
    public function tableName(){
        return 'businessloans_feed_vendor';
    }
    /**
    ...
    public function listfeedbrowseleads(){
        ...
        $rawData = Yii::app()->dbBusinessLoans->createCommand()->select($string)->from('businessloans_feed_submission a_sub')->join('businessloans_feed_vendor_transactions a_ft', 'a_sub.id=a_ft.customer_id')->join('businessloans_feed_vendor a_afu', 'a_ft.feed_vendor_id  = a_afu.id')->join('businessloans_feed_lender_transactions a_lts', 'a_lts.feed_vendor_transactions_id   =  a_ft.id')->where($where)->queryAll();
        return $rawData;
    }
    
    public function listfeedbrowseallleads(){
        $string = "a_sub.*";
        $rawData = Yii::app()->dbBusinessLoans->createCommand()->select($string)->from('businessloans_feed_submission a_sub')->join('businessloans_feed_vendor_transactions a_ft', 'a_sub.id=a_ft.customer_id')->join('businessloans_feed_vendor a_afu', 'a_ft.feed_vendor_id  = a_afu.id')->join('businessloans_feed_lender_transactions a_lts', 'a_lts.feed_vendor_transactions_id   =  a_ft.id')->queryAll();
        //echo Yii::app()->dbBusinessLoans->get_query();exit;
        return $rawData;
    }
