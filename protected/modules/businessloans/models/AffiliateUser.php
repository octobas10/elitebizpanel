...
/**
 * This is the model class for table "businessloans_affiliate_user".
 *
 * The followings are the available columns in table 'businessloans_affiliate_user':
 * @property string $id
 ...
 */
class AffiliateUser extends BusinessloansActive {
	/**
	...
	public function tableName(){
		return 'businessloans_affiliate_user';
	}
	/**
	...
	public function check_affiliate_cap($promo_code){
		...
		if(isset($row['cap_limit']) && $row['cap_limit']==-1){
			...
			$total_submission_of_affiliate_today = Yii::app()->dbBusinessLoans->createCommand()->select('COUNT(id)')->from('businessloans_submissions')->where($where)->queryScalar();
	...
	public function update_bucket_and_return_pixel_count(){
		...
		$update = Yii::app()->dbBusinessLoans->createCommand()->update('businessloans_affiliate_user', array('bucket'=>$bucket), 'id=:id', array(':id'=>$promo_code));
		...
		if($update){
			while($bucket>=($bucket_limit)){
				$bucket -= $bucket_limit;
				$pixel++;
				Yii::app()->dbBusinessLoans->createCommand()->update('businessloans_affiliate_user', array('bucket'=>$bucket,'pixel_count'=>$pixel_count+1),'id=:id',array(':id'=>$promo_code));
			}
		}
		return $pixel;
	}
