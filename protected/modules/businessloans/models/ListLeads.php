<?php
class ListLead extends BusinessloansActive
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('home_pay','required','message'=>'Rent / businessloans required'),
	...
	public function checkDuplicate($data){
		...
		SELECT COUNT(id) as count FROM `businessloans_submissions` WHERE email='".$data['email']."' AND first_name = '".$data['first_name']."' AND last_name ='".$data['last_name']."' AND UNIX_TIMESTAMP(sub_date) > '".@strtotime('-1 month')."' ");
