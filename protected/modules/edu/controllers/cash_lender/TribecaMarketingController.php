<?php
class TribecaMarketingController extends Controller {
    ...
    public static function sendPostData($parameter1,$parameter2,$parameter3,$ping_response,$post_url,$status){
		...
		$program_model = new ProgramOfInterests();
		$valid_program = $program_model->checkProgramOfInteresetTribecaMarketing(Yii::app()->request->getParam('program_of_interest'));
		if($valid_program['id']){
		...
		$promo_code = Yii::app()->request->getParam('promo_code');
		$lead_source_id = ($promo_code=='109' || $promo_code=='111' || $promo_code=='112' || $promo_code=='113' ) ? '2919' : '2860';
    	$fields = array(
		...
		}else if (strpos($post_full_response, 'Duplicate') !== false || strpos($post_full_response, 'multiple times') !== false){
			$post_status = '2';
			$post_price = '0';
			$redirect_url = '';
		}else {
