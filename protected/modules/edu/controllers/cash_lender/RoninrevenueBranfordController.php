<?php
class RoninrevenueBranfordController extends Controller {
    ...
    public static function sendPostData($parameter1,$parameter2,$parameter3,$ping_response,$post_url,$status){
		...
		$program_model = new ProgramOfInterests();
		$valid_program = $program_model->checkProgramOfInteresetRoninRevenuesBrandfordSchool(Yii::app()->request->getParam('program_of_interest'));
		if($valid_program['id']){
		...
		if($master_degree){
			$education_level_id = '169';
		}else if($ged){
			$education_level_id = '165';
		}else{
			$EDU_array = array('166','167','168');
		...
		$promo_code = Yii::app()->request->getParam('promo_code');
    	$fields = array(
			'campaign_code' => $parameter1 ? $parameter1 :'xY3RAJzPUMCH4MdAl6l6gQ',
			...
			'lead_education[campus_id]' => $campus_id_rr,
