<?php
class DegreeAmericaController extends Controller {
    ...
    public static function returnPingData($p1 = false, $p2 = false, $p3 = false, $p4 = false){
		...
		$enroll = array(0,1,4);
		$en_key = array_rand($enroll);
        $fields = array(
			'auth' => $p1,
            'salutation' => Yii::app()->request->getParam('salutation'),
			...
			'phone1[area]'		=> substr($phone1,0,3),
    		'phone1[prefix]'	=> substr($phone1,3,3),
    		'phone1[suffix]'	=> substr($phone1,6,4),
    		'phone2[area]'		=> substr($phone2,0,3),
    		'phone2[prefix]'	=> substr($phone2,3,3),
    		'phone2[suffix]'	=> substr($phone2,6,4),
    		'age' => $age,
    		'interest' => $interest,
    		'subject' => $subject,
    		'education_level' => $edu_level,
    		'degree_level' => $degree_level,
			'enroll' => $enroll[$en_key],
			'campus_pref' => 'Either',
			'computer_access' => 'yes',
			'us_citizen' => 'yes',
			'military_affil' => 'no',
			'rn_license' => Yii::app()->request->getParam('are_you_a_registered_nurse'),
			'teaching' => Yii::app()->request->getParam('do_you_have_a_teaching_certificate'),
			'international' => 'no',
			'leadid_token' => Yii::app()->request->getParam('universal_leadid'),
			'sub_source' => Yii::app()->request->getParam('promo_code'),
        );
    ...
    public static function sendPostData($parameter1,$parameter2,$parameter3,$ping_response,$post_url,$status){
		...
		$program_model = new ProgramOfInterests();
		$valid_program = $program_model->checkProgramOfInteresetDegreeAmerica(Yii::app()->request->getParam('program_of_interest'));
		//echo '<pre>123';print_r($valid_program);exit;
		if($valid_program['dm_subject']){
			$prog_of_int = $valid_program['dm_subject']; //Yii::app()->request->getParam('program_of_interest');
		}else{
		...
		switch($education_level){
			case '1';
				$edu_level = 'GED';
				$desired_degree_needed = array('associate','certificate','diploma');
				break;
			case '2';
				$edu_level = 'High School';
				$desired_degree_needed = array('associate','certificate','diploma','bachelor');
				break;
			case '3';
				$edu_level = '1-23 Credits';
				$desired_degree_needed = array('associate','certificate','diploma','bachelor');
				break;
			case '4';
				$edu_level = '24-47 Credits';
				$desired_degree_needed = array('associate','certificate','diploma','bachelor');
				break;
			case '5';
				$edu_level = '48+ Credits';
			...
			case '6';
				$edu_level = "associate";
				$desired_degree_needed = array('certificate','diploma','bachelor');
				break;
			case '7';
				$edu_level = 'bachelor';
				$desired_degree_needed = array('master','certificate','diploma');
				break;
			case '8';
				$edu_level = 'master';
				$desired_degree_needed = array('doctoral','certificate','diploma');
				break;
			case '9';
				$edu_level = 'doctoral';
				$desired_degree_needed = array('doctoral','certificate','diploma');
    	...
    	$desired_degree = Yii::app()->request->getParam('desired_degree');
		switch($desired_degree){
			case '1';
				$degree_level ='diploma';
				break;
			case '2';
		...
		if(!in_array($degree_level,$desired_degree_needed)){
			$key_ddn = array_rand($desired_degree_needed);
			$degree_level = $desired_degree_needed[$key_ddn];
		}
    	$pingData = array();
		$phone1 = Yii::app()->request->getParam('phone');
        $phone2 = Yii::app()->request->getParam('mobile');
		...
		$en_key = array_rand($enroll);
        $fields = array(
			'auth' => $p1 ? $p1 : '9228339c89138475c2d86ad4d6574e91',
            'salutation' => Yii::app()->request->getParam('salutation'),
            'first_name' => Yii::app()->request->getParam('first_name'),
            'last_name' => Yii::app()->request->getParam('last_name'),
			'email' => Yii::app()->request->getParam('email'),
    		...
    		'phone2[suffix]'	=> substr($phone2,6,4),
    		'age' => $age,
    		'interest' => $valid_program['dm_interest'],
    		'subject' => $valid_program['dm_subject'],
    		'education_level' => $edu_level,
    		'degree_level' => $degree_level,
			'enroll' => $enroll[$en_key],
			'campus_pref' => $campus_id_rr,
			'computer_access' => 'yes',
			'us_citizen' => 'yes',
			'military' => 'None',
			'military_affil' => 'no',
			'rn_license' => $rn_license == 0 ? 'no' : 'yes',
			'teaching' => $teaching == 0 ? 'no' : 'yes',
			'international' => 'no',
			'leadid_token' => Yii::app()->request->getParam('universal_leadid'),
			'sub_source' => Yii::app()->request->getParam('promo_code'),
        );
		/* ============== PING START FROM HERE ================= */
		$ping_request = '';
		/*foreach ($fields as $key => $value) $ping_request .= '&'.$key.'='.urlencode($value);
		$ping_request = substr($ping_request,1);*/
		$ping_request = '';
		$ping_request = http_build_query($fields);
		
    	$cm = new CommonMethods();
    	$start_time = CommonToolsMethods::stopwatch();
    	$ping_url = 'https://degreeamerica.com/consumer/search.php';
    	$ping_full_response = $cm->curl($ping_url,$ping_request);
		//$ping_full_response ='{"universities":{"546":{"name":"Remington College Campus","school":"Cleveland","logo":"\/images\/content\/schools-big\/logo_remington.gif","programs":[{"id":"1564279","name":"Heating, Ventilation and Air Conditioning","school":"Cleveland","degree":"Diploma","online":"0","school_id":"9442"},{"id":"1564234","name":"Cosmetology Degree","school":"Cleveland","degree":"Associate","online":"0","school_id":"9442"},{"id":"1564267","name":"Facility Maintenance and Technology","school":"Cleveland","degree":"Associate","online":"0","school_id":"9442"},{"id":"1564286","name":"HVAC Associate Degree 24 mo","school":"Cleveland","degree":"Associate","online":"0","school_id":"9442"}],"description":"Remington College New Build Setup (YNot)","additional_questions":[{"type":"checkbox","name":"remington_tcpa","label":"I acknowledge that, by clicking the checkbox as my official signature, I consent to representatives of Remington College contacting me about educational opportunities via email, text, or phone, including my mobile phone if provided above, using an automatic dialer, or pre-recorded message. Message and data rates may apply. I understand that my consent is not a requirement for enrollment, and that I may withdraw my consent at any time.","options":"{\"label\": \"TCPA\",\"value\": \"yes\"}"}]}},"interest_id":5953479}';
		...
		$ping_status = isset($ping_json_response->universities) ?  1 : 0 ;
		
		$ping_time = ($time_end - $start_time);
		$ping_id = $cm->setLenderTransactionLogEdu('DegreeAmerica',24.00,null,null,null,null,$ping_request,$ping_status,$ping_full_response,$ping_time,$redirect_url=null,$ping_id=null,$campus_code,$lender_id);
		$flag_program = false;
		if($ping_status == 1){
			$interest_id = $ping_json_response->interest_id;
			foreach ($ping_json_response->universities as $university_id => $colleges) {
				$university_id = $university_id;
				foreach ($colleges->programs as $college) {
					if(strtolower($college->degree) == $degree_level){
						$programs_id = $college->id;
						$flag_program = true;
					}
					$alt_programs_id = $college->id;
				}
				if(!$flag_program){
					$programs_id = $alt_programs_id;
				}
				foreach ($colleges->additional_questions as $additional_questions) {
					$additional_questions_field[] = $additional_questions->name; 
					$options = json_decode($additional_questions->options);
					if(is_array($options)){
						foreach ($options as $option) {
							$option_obj =  $option;
						}
						$additional_questions_value[] =  $option_obj->value;
					}else{
						$additional_questions_value[] =  $options->value;
					}
				}
			}
			$campus_code = $_REQUEST['campus'];
			...
			foreach ($additional_questions_field as $key => $field) {
				$additional_questions[$field] = $additional_questions_value[$key];
			}
			$fields = array_merge($fields,$additional_questions);
			$start_time = CommonToolsMethods::stopwatch();
			$post_request = '';
			...
			//$post_full_response = '{"result":"success"}';
			$pstatus = json_decode($post_full_response);
			$time_end = CommonToolsMethods::stopwatch();
			if (trim($pstatus->result) == 'success' || trim(strtolower($pstatus->result)) =='success') {	
				$post_status = '1';
		...
		//echo '<pre>...';print_r($post_responses);exit;
		//exit();
