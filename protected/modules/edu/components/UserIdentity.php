...
class UserIdentity extends CUserIdentity{
	...
	public function affiliate_authenticate(){
		...
		if($record === null)
			...
			Yii::app()->user->setState('userinactive',true);
		else{
			$this->id = $record->id;
	...
	public function feedlender_authenticate(){
		$record = EduFeedLenders::model()->findByAttributes(array(
			'feed_lender_name' => $this->username 
		));
		if($record === null)
			$this->errorCode = self::ERROR_USERNAME_INVALID;
		else if($record->password !== md5($this->password))
			...
			$this->id = $record->id;
			$this->setState('usertype','edulender');
			$this->errorCode = self::ERROR_NONE;
		}
		return !$this->errorCode;
	}
