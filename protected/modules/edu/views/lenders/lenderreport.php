...
<tr>
	...
	<?php echo Chtml::dropDownList('lender', $lender,
			...
			array('style'=>'width:auto;','empty'=>'All Lenders','class'=>'inputClass'));
	?>
	</td>
	<td><b>Action :</b><br>
...
<thead>
	...
	<th>Returned</th>
	<th>Grand Total</th>
	<th>Average Turn Over<!--  / Profit Sum --></th>
	</tr>
</thead>
<tbody>
<?php
...
$buyer_leads = 0;$buyer_lead_price=0;$buyer_lead_margin=0;$buyer_leads_return=0;$buyer_turn_over=0;$buyer_profit=0;
	foreach ($lender_lead_price as $lead_prices){
		$buyer_lead_price = $lead_prices['lead_price'];
		$buyer_lead_margin = $lead_prices['margin'];
		echo '<tr>';
		if($i == 0){
			echo '<td '.$rowspan.' width="120px;">'.$lender.'</td>';
		}
		echo '<td width="120px;">$'.$lead_prices['lead_price'].'</td>';
		echo '<td width="120px;"><a target="_blank" href="'.Yii::app()->createAbsoluteUrl(Yii::app()->params['campaign']).'/leads/lead_info?lead_price='.$lead_prices['lead_price'].'&lender='.$lender.'&start_date='.$start_date.'&end_date='.$end_date.'">'.$lead_prices['leads'].'</a></td>';
		if($i == 0){
			$buyer_leads = $lender_total[$lender]['leads'];
			echo '<td '.$rowspan.' width="120px;"><a target="_blank" href="'.Yii::app()->createAbsoluteUrl(Yii::app()->params['campaign']).'/leads/lead_info?&lender='.$lender.'&start_date='.$start_date.'&end_date='.$end_date.'">'.$buyer_leads.'</a></td>';
			$buyer_leads_return = $lender_total[$lender]['returned'];
			echo '<td '.$rowspan.' width="120px;"><a target="_blank" href="'.Yii::app()->createAbsoluteUrl(Yii::app()->params['campaign']).'/leads/lead_info?&lender='.$lender.'&start_date='.$start_date.'&end_date='.$end_date.'&is_returned=1">'.$buyer_leads_return.'</td>';
			echo '<td '.$rowspan.' width="120px;"><a target="_blank" href="'.Yii::app()->createAbsoluteUrl(Yii::app()->params['campaign']).'/leads/lead_info?&lender='.$lender.'&start_date='.$start_date.'&end_date='.$end_date.'&final=1">'.($buyer_leads-$buyer_leads_return).'</a></td>';
			// LAST COLUMN
			$buyer_turn_over = (($buyer_leads-$buyer_leads_return)*$lead_prices['lead_price']);
			$buyer_profit = (($buyer_leads-$buyer_leads_return)*$lead_prices['lead_price'])/$buyer_lead_margin;
			echo '<td '.$rowspan.' width="120px;">$'.number_format($buyer_turn_over,2).' / $'.number_format($buyer_profit,2).'</td>';
		}
		echo '</tr>';
		$i++;
	}
	$total_leads += round($buyer_leads,2);
