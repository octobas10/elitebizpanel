...
<tr>
	...
	<br/><br/><br/>
		...
		<?php echo CHtml::radioButtonList('lead_status', ''.$lead_status.'', 
			array(1 => 'ACCP.' , 0 => 'DECLINED'),
			array('labelOptions'=>array('style'=>'display:inline')));
		?></br></br>
		<b>Returned :</b></br>
		<?php echo CHtml::radioButtonList('is_returned', ''.$is_returned.'', 
			array(0 => 'NON-RET' , 1 => 'RETUNED', -1 => 'BOTH'),
			array('labelOptions'=>array('style'=>'display:inline')));
		?>
	</td>
	<td><b>Lenders :</b><br>
	<?php echo Chtml::listBox('lenders[]', $lenders,
			...
			array('style'=>'width: 190px;','size'=>'15','empty'=>'All Lenders'));
	?>
	</td>
	...
	<b>Postal Verified?</b><br>
	<?php echo CHtml::radioButtonList('USPS_postal_verified', ''.$USPS_postal_verified.'', 
			array('' => 'None','1' => 'Yes', '0' => 'No'), 
			array('labelOptions'=>array('style'=>'display:inline')));
	?>
	</td>	
	...
	<b>xVerified Email?</b><br>
	<?php echo CHtml::radioButtonList('xverify_email', ''.$xverify_email.'', 
			array('' => 'None','1' => 'Yes', '0' => 'No'), 
			array('labelOptions'=>array('style'=>'display:inline')));
	?>
	</td>
...
<tr>
	...
	/**
	  ...
	  * @description : IP Address field commented
	  * @since : 14-04-2017 12:40
	 */
	 -->
    <th>IP Address</th> 
	<th>First Name</th>
...
foreach ($posts as $sub_leads) { ?>
	 ...
	 */
	 -->
	<td> <?php echo urldecode($sub_leads['ipaddress']); ?>
	<?php 
		/**
		 * @since : 22-12-2016 04:24 PM
		...
		$t_ip_location = json_decode(file_get_contents("https://ipinfo.io/{$sub_leads['ipaddress']}/geo", false, stream_context_create($arrContextOptions)));
		//$t_ip_location = json_decode(file_get_contents("https://ipinfo.io/{$sub_leads['ipaddress']}/geo"));
	...
	<td><?php echo urldecode($sub_leads['gender'])." (".$sub_leads['promo_code'].")";//$model = AffiliateUser::model()->findByPk($sub_leads['promo_code']); echo $model->user_name.' ('.$sub_leads['promo_code'].')';?></td>
	<td><?php echo ($sub_leads['is_returned']==1) ? RETURNED : setResponseText($sub_leads['lead_status'])?></td>
	<td><?php echo urldecode($sub_leads['lender_id']); ?></td>
