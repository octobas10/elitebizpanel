...
/**
 * This is the model class for table "edu_feed_vendor".
 *
 * The followings are the available columns in table 'edu_feed_vendor':
 * @property integer $id
 ...
 */
	...
	*/
	 ...
	 ** Modification Date : 02-08-2016
	**/
class EduFeedVendors extends EModuleActiveRecord {
    /**
     * @return string the associated database table name
     */
    public function tableName(){
        return 'edu_feed_vendor';
    }
    /**
    ...
    public function listfeedbrowseleads(){
        ...
        $rawData = parent::getDbConnection()->createCommand()->select($string)->from('edu_feed_submission a_sub')->join('auto_feed_vendor_transactions a_ft', 'a_sub.id=a_ft.customer_id')->join('edu_feed_vendor a_afu', 'a_ft.feed_vendor_id  = a_afu.id')->join('auto_feed_lender_transactions a_lts', 'a_lts.feed_vendor_transactions_id   =  a_ft.id')->where($where)->queryAll();
        return $rawData;
    }
    
    public function listfeedbrowseallleads(){
        $string = "a_sub.*";
        $rawData = parent::getDbConnection()->createCommand()->select($string)->from('edu_feed_submission a_sub')->join('edu_feed_vendor_transactions a_ft', 'a_sub.id=a_ft.customer_id')->join('edu_feed_vendor a_afu', 'a_ft.feed_vendor_id  = a_afu.id')->join('auto_feed_lender_transactions a_lts', 'a_lts.feed_vendor_transactions_id   =  a_ft.id')->queryAll();
        //echo parent::getDbConnection()->get_query();exit;
        return $rawData;
    }
    ...
    public function checkVendorStatus(){
        ...
        $row = EduFeedVendors::model()->find($criteria);
        if($row['status'] != '1'){
